...
import java.lang.*;
public class serverCashier extends UnicastRemoteObject implements interfaceRailway
{
        Connection connection,connectionD;
        Statement st,st1,stD;
        ResultSet rs,rs1,rsD;
        static String finYear=null;
        public serverCashier() throws RemoteException{}
        {
        ...
        //	boolean bn=false;
        	...
        	{ 
                ...
                DBCon=java.sql.DriverManager.getConnection("jdbc:mysql://localhost/p3manager"+fin+"?user=p3&password=admin");
        ...
        public boolean selectFinancialYear(String cnm,String fin)
        {
        ...
        java.sql.Connection DBCon=null;
        	...
        	{ 
        		...
        		System.out.println("a");     	
	        	Class.forName("com.mysql.jdbc.Driver").newInstance(); 
                System.out.println("p3manager"+fin);
                // connection= DriverManager.getConnection("jdbc:mysql://localhost/railway"+fin+"?user=RailwaysCashOffice&password=RCO");
                System.out.println("a");
                DBCon=java.sql.DriverManager.getConnection("jdbc:mysql://localhost/"+cnm+fin+"?user=p3&password=admin");
             //   System.out.println("jdbc:mysql://localhost/railway"+fin+"?user=cashier&password=rnr");
           	//	st = DBCon.createStatement(); 
           	//	st1 = DBCon.createStatement();
           		finYear=fin; 
        ...
        public boolean checkFinancialDB(String path)
        {
        				...
        				else
        				{
        					...
        					boolean bl=fl.mkdir();     
          					...
          					fl=new File("C:\\mysql\\data\\p3managerblankdatabase");
        					String fname[]=fl.list();
        		                              //Copy blank database
        		            for(int i=0;i<fname.length;i++)                  
                            {
                            	try
                                {
                                	    FileInputStream in = new FileInputStream("C:\\mysql\\data\\p3managerblankdatabase\\"+fname[i]);
...
/*	public int getCountValue(String query)
	...
	public String getBackupOfDatabase()
    {
    	...
    	try
    	{
            ...
            String filename=dirName+"\\"+folderName+"\\"+toDayIs+"\\p3manager"+finYear;//+"\\"+"hospitalMNG.mdb";
            ...
            String dataPath="c:\\mysql\\data\\"+"p3manager"+finYear;
...
/*        public boolean executeSQLUpdateCommand(String url)
        ...
        public static void main(String args[])
        {
                try
                {
                        serverCashier server1=new serverCashier();
                        Naming.rebind("railwayMethods",server1);
                }
                catch(Exception x)
                {
