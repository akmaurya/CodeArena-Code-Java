 ...
 * @author  Deep Pal
 */
public class MakingSaleToPatient extends javax.swing.JInternalFrame {
    
    /** Creates new form MakingSale */
    public MakingSaleToPatient(javax.swing.JDesktopPane jDesktopPane1) {
        initComponents();
        boolean flag=true,flag1=true;
        int bedNo=0;
        
        
        System.out.println("this is the file     shwetaaaaaaaaaaa");
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            if(arr!=null)
            {
            	...
            	NM=inf.getSingleValue("select name from patient_Profile where patientid="+patientID);
            	System.out.println("Name   "+NM);
            	lbl_Label.setText(""+patientID+"#"+NM);
            	Addr=inf.getSingleValue("select address from patient_Profile where patientid="+patientID);
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
        ...
        else        //checking stock of item
        {
            ...
            String stockQ=inf1.getSingleValue("select qty from stockHolding where itemCode="+itemCode+" and batchNo='"+jTextField3.getText()+"'");
            if(stockQ!=null)
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            if(flag==0)
            {
                ...
                int sno=Integer.parseInt(""+uc.getSaleNo());
            //java.util.Date dt=new java.util.Date();
            ...
            boolean treatedAs=true;
            int max=inf.getMaxValue("select max(receiptNo) from patient_All_Invoice where status=0");
			max+=1;
            if(lbl_Label.getText().equalsIgnoreCase("TO HOSPITAL"))
            {
            	inf.executeUpdateQuery("insert into saleTo_HPS_Patient values("+sno+",0)");
            	//----send data into invoice----
						String query="insert into patient_All_Invoice values("+max+",#"+entrydate+"#,-2,'Through Medical(To Hospital)',"+netamt+",0)";
						inf.executeUpdateQuery(query);
            }
            else
            {
            	String s[]=lbl_Label.getText().split("#");
            	inf.executeUpdateQuery("insert into saleTo_HPS_Patient values("+sno+","+Integer.parseInt(s[0])+")");
            	//----send data into invoice----
						String query="insert into patient_All_Invoice values("+max+",#"+entrydate+"#,"+Integer.parseInt(s[0])+",'Through Medical(To Patient)',"+netamt+",0)";
						inf.executeUpdateQuery(query);
            	treatedAs=false;
            }
            for(int r=0;r<dtm1.getRowCount();r++)
            {
                ...
                if(qty==0)
                    ...
                    inf.executeUpdateQuery("delete * from stockHolding where itemCode="+itemcode+" and batchno='"+btno+"'");
                else
...
catch(Exception e)
{
	...
	{
		System.out.println("Name     in print"+NM);
		try
		{
			String space="";
            ...
            try
            {
              ...
              /*  fos=new java.io.FileOutputStream("System.in");
                System.out.println("inside find port...LPT1");*/
