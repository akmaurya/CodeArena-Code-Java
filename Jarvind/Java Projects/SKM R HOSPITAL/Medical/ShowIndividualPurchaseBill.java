 ...
 * MakePurchase.java
 *
 * Created on April 8, 2010, 9:15 AM
 */
 ...
 * @author  Deep Pal
 */
public class ShowIndividualPurchaseBill extends javax.swing.JInternalFrame {
    
    /** Creates new form MakePurchase */
    public ShowIndividualPurchaseBill(javax.swing.JDesktopPane jDesktopPane1,int pNo,String callFrom) {
        initComponents();
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            String qry="";
            if(callFrom.equalsIgnoreCase("purchaseSmry"))
            {
                qry="select partyNm,partyAds,bookNo,srNo,entrydt,totalitems,totalAmt,roAmt,netAmt,wholeTypeOfDrug,wholeTypeOfPay from purchaseSmry where pNo="+pNo;
            }
            else if(callFrom.equalsIgnoreCase("purchaseSmryEdit"))
            {
                qry="select partyNm,partyAds,bookNo,srNo,entrydt,totalitems,totalAmt,roAmt,netAmt,wholeTypeOfDrug,wholeTypeOfPay from purchaseSmryEdit where editNo="+pNo;
            }
            String [][]rs=inf.getResultSetAs_2D_Array(qry,11);
            if(rs!=null)
            {
                jTextField1.setText(rs[0][0]);
                jTextArea1.setText(rs[0][1]);
                jTextField2.setText(rs[0][2]);
                jTextField3.setText(rs[0][3]);
                String dat[]=rs[0][4].split("-");
                ...
                jLabel22.setText(rs[0][5]);
                jLabel26.setText(rs[0][6]);
                jLabel27.setText(rs[0][7]);
                jLabel28.setText(rs[0][8]);
                cb_typeOfDrug.setSelectedItem((rs[0][9]));
                cb_typeOfPay.setSelectedItem((rs[0][10]));
                
                if(callFrom.equalsIgnoreCase("purchaseSmry"))
                {
                    qry="select * from purchaseDtl where pNo="+pNo;
                }
                else if(callFrom.equalsIgnoreCase("purchaseSmryEdit"))
                {
                    qry="select * from purchaseDtlEdit where editNo="+pNo;
                }
                System.out.println(""+qry);
                String [][] rs1=inf.getResultSetAs_2D_Array(qry,13);
                if(rs1!=null)
                for(int i1=0;i1<rs1.length;i1++)
                {
    ...
    private void initComponents() {
        ...
        jTextField1.setBounds(120, 20, 230, 19);
        ...
        jTextField2.setBounds(90, 50, 80, 19);
        ...
        jTextField3.setBounds(240, 50, 110, 19);
        ...
        jTextField4.setBounds(70, 50, 110, 19);
        ...
        jTextField5.setBounds(430, 50, 70, 19);
        ...
        jTextField6.setBounds(670, 50, 50, 19);
        ...
        jTextField7.setBounds(730, 50, 40, 19);
        ...
        jTextField8.setBounds(110, 80, 80, 19);
        ...
        jTextField9.setBounds(280, 80, 40, 19);
        ...
        jButton2.setBounds(700, 70, 30, 30);
        ...
        expDateLb.setBounds(260, 50, 90, 19);
        ...
        jTextField10.setBounds(100, 20, 190, 19);
        ...
        jTextField11.setBounds(350, 20, 220, 19);
        ...
        jButton6.setBounds(740, 70, 30, 29);
        ...
        jButton4.setBounds(660, 433, 130, 30);
        ...
        jButton7.setBounds(760, 0, 20, 20);
...
// TODO add your handling code here:
        if(evt.getKeyCode()==10)    jTextField10.requestFocus();
...
catch(Exception e)
{
	...
	try
	{
		...
		obj=(ZedInterfaceForMedical)Naming.lookup("rmi://127.0.0.1/ExtendZedServer");
	}
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
        ...
        String itemNm[]=inf1.getItemNames(jTextField10.getText());//""+coNmVc.elementAt(jList1.getSelectedIndex()));
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            //-----------------------
                String [][]rs1=inf.getResultSetAs_2D_Array("select itemNm,batchNo,qty from purchaseDtl where pNo="+pNo,3);
                if(rs1!=null)
                for(int i=0;i<rs1.length;i++)
                {
                    String itemname=rs1[i][0];
                    String batchno=rs1[i][0];
                    int quantity=Integer.parseInt(rs1[i][2]);
                    int itemcode=uc.getItemCode(itemname);
                    
                    String typeofitem="";
                    String [][]rs2=inf.getResultSetAs_2D_Array("select typeOfItem,pkging from itemMaster where itemCode="+itemcode,2);
                    if(rs2!=null)
                    //if(rs2.next())
                    {
                        typeofitem=rs2[0][0];
                        ...
                        {
                            quantity=quantity * Integer.parseInt(rs2[0][1]);
                        }
                    }
                
                    String eqty=inf.getSingleValue("select qty from stockHolding where itemCode="+itemcode+" and batchNo='"+batchno+"'");
                    //rs2=st2.executeQuery("select qty from stockHolding where itemCode="+itemcode+" and batchNo='"+batchno+"'");
                    if(eqty!=null)
                    {
                        quantity=Integer.parseInt(eqty)-quantity;
                ...
                //st1.executeUpdate("delete * from purchaseDtl where pNo="+pNo);
                //st1.executeUpdate("delete * from purchaseSmry where pNo="+pNo);
                inf.executeUpdateQuery("delete * from purchaseDtl where pNo="+pNo);
                inf.executeUpdateQuery("delete * from purchaseSmry where pNo="+pNo);
                
            //insert new records
            //------------------    
            //int pno=ut.getNextPurchaseNo();
            ...
            inf.executeUpdateQuery("insert into purchaseSmry values("+pno+",'"+entrydate+"','"+partynm+"','"+partyads+"','"+bookno+"','"+srno+"',"+totalitems+","+totalamt+","+roamt+","+netamt+")");
            for(int r=0;r<dtm2.getRowCount();r++)
            {
                String coname=""+jTable2.getValueAt(r,1);
                String btno=""+jTable2.getValueAt(r,2);
                ...
                if(rs.next())
                {
                    ...
                    qty=qty+existingQty;
                    st.executeUpdate("update stockHolding set qty="+qty+",expDate='"+expdate+"' where itemCode="+itemcode+" and batchNo='"+btno+"'");
                }
                else
                {
                ...
                {
                    //typeofitem
                    inf.executeUpdateQuery("insert into stockHolding values("+itemcode+","+qty+",'"+uc.getSmallestQtyType(itemnm)+"','"+btno+"','"+expdate+"')");
                    //st.executeUpdate("insert into stockHolding values("+itemcode+","+qty+",'"+ut.getSmallestQtyType(itemnm)+"','"+btno+"','"+expdate+"')");
                }
