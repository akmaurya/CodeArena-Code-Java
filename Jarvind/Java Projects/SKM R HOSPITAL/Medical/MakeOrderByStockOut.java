 ...
 * @author  Deep Pal
 */
public class MakeOrderByStockOut extends javax.swing.JInternalFrame {
    
    /** Creates new form MakePurchase */
    public MakeOrderByStockOut(javax.swing.JDesktopPane jDesktopPane1,int x,int y,String mainItem,String itemList[],int orderNo) {
        initComponents();
        ...
        {
            if(mainItem.equalsIgnoreCase(itemList[i]))
            {
                continue;
            }
            itemList[i]=itemList[i];
        }
        itemList[i-1]=mainItem;
        */
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            String query="select max(oNo) from orderSmry";
            int orderNo=inf.getMaxValue(query);
            orderNo+=1;
    ...
    private void showInitialInfo()
    {
        try
        {
            ...
            //ZedInterface inf=(ZedInterface)Naming.lookup(getIPaddress.IP());
                String query="select pNo from purchaseDtl where itemNm='"+mainItem+"' order by pNo desc";
                showInitialInfo1(query);
            
            
        }   
        catch(Exception e)
        {
            System.out.println("Error in showIntialInfo method in MakeOrderByStockOut class..."+e);
        }
    }
    private void showInitialInfo1(String query)
    {
        try
        {
            boolean flag=false;
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            if(intializeNameAndAddress && flag)
            {
                intializeNameAndAddress=false;
                query="select partyNm,partyAds from purchaseSmry where pNo="+pNo;
                showInitialInfo2(query);
        ...
        {
            System.out.println("Error in showInitialInfo1 method in MakeOrderByStockOut class..."+e);
        }
    }
...
catch(Exception e)
{
           ...
           if(rs!=null)
            for(int ind=0;ind<rs.length;ind++)
            {
                ...
                for(int i=0;i<itemList.length;i++)
                {
                    ...
                    {
                        //query="select *from purchaseDtl where "
                        String data[]=new String[4];
                        data[0]=""+(jTable2.getRowCount()+1)+". ";
                        data[1]=company;
                        data[2]=item;
                        ...
                        boolean checkLastParty=checkingPurchaseOfItemByLastParty(query);//u
                        if(checkLastParty)//u
                        {//u
                            dtm2.addRow(data);
                            itemList[i]="no use";
    ...
    private void showInitialInfo2(String query)
    {
        try
        {
            /*ConnectDatabase cd=new ConnectDatabase();
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            {
                pNo=Integer.parseInt(rs);
				query="select pNo,coNm,itemNm,qty from purchaseDtl where pNo="+pNo+"  order by pNo desc";
                takingListOfItem(query);
            }
            //connection.close();
        }   
    ...
    private boolean checkingPurchaseOfItemByLastParty(String query)
    {
        boolean flag=true;
        try
        {
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
        ...
        {
            System.out.println("Error in checkingPurchaseOfItemByLastParty method in OutStockReport class..."+e);
        }
        return flag;
    }
    ...
    String mainItem;
    int purchaseNo;
    boolean intializeNameAndAddress=true;
}
