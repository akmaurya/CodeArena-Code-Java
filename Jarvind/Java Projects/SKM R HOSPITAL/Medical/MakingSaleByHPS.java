 ...
 * @author  Deep Pal
 */
public class MakingSaleByHPS extends javax.swing.JInternalFrame {
    
    /** Creates new form MakingSale */
    public MakingSaleByHPS(javax.swing.JDesktopPane jDesktopPane1) {
        initComponents();
        boolean flag=true,flag1=true;
        int bedNo=0;
        System.out.println("this is the file");
        String pid=JOptionPane.showInputDialog(null,"Enter Bed No.");
        if(pid!=null)
        {
        	try
        	{
        		bedNo=Integer.parseInt(pid);	
        		flag1=false;
        	}
        	catch(Exception e)
        	{
        		System.out.println("Error in constuctor of MakingSaleToPatient class...."+e);
        		flag=false;
        	}
        if(flag)
        {
        //---new code added here------------------------------------------------------
        lbl_Label=new javax.swing.JLabel(pid);
        lbl_Label.setBackground(new java.awt.Color(130, 251, 124));
        lbl_Label.setFont(new java.awt.Font("Microsoft Sans Serif", 1, 14));
        lbl_Label.setForeground(new java.awt.Color(83, 0, 41));
        lbl_Label.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        lbl_Label.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(255, 0, 0)));
        lbl_Label.setOpaque(true);
        getContentPane().add(lbl_Label);
        lbl_Label.setBounds(170, 10, 350, 20);
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            String arr=inf2.getSingleValue("select id from admit_Patient_Info where bedNo="+bedNo+" and status=true");
            if(arr!=null)
            {
            	int patientID=Integer.parseInt(arr);
            	NM=inf2.getSingleValue("select name from patient_Profile where patientid="+patientID);
            	lbl_Label.setText(""+patientID+"#"+NM);
            	Addr=inf2.getSingleValue("select address from patient_Profile where patientid="+patientID);
            	System.out.println("Addr  "+Addr);
            }
            else
            {
            	JOptionPane.showMessageDialog(null,"You Have Wrong Bed Number\nPlease Input Correct Bed No.");
            	flag=false;
            }
            }
            catch(Exception e)
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
        ...
        int saleNo=uc.getMaxValue("select max(sNo) from saleSmry_ByHPS");
        saleNo+=1;
        jLabel33.setText(""+saleNo);
        }
        catch(Exception ex)
		{
    ...
    private void initComponents() {
        ...
        jPanel2.setBounds(480, 80, 290, 220);
        ...
        jLabel35.setBounds(600, 10, 130, 20);
        ...
        jButton3.setBounds(20, 370, 130, 90);
        ...
        jButton4.setBounds(160, 400, 260, 30);
        ...
        jButton5.setBounds(160, 430, 260, 30);
        ...
        jButton6.setBounds(720, 90, 30, 30);
        ...
        jButton7.setBounds(740, 10, 20, 20);

        jButton8.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icons/PRINT.GIF")));
        ...
        jButton8.setBounds(160, 370, 260, 30);
...
// TODO add your handling code here:
        if(evt.getKeyCode()==10)    clearFullComponents();
...
// TODO add your handling code here:
        ...
        try
        {
        ZedInterface inf1=null;
ZedInterfaceForMedical uc=null;
try
{
	inf1=(ZedInterface)Naming.lookup(getIPaddress.IP());
	uc=(ZedInterfaceForMedical)Naming.lookup(getIPaddress.IP());
}
catch(Exception e)
{
	...
	catch(Exception ex)
	{
        ...
        else        //checking stock of item
        {
            ...
            String cmpC=inf1.getSingleValue("select cmpCode from cmpMaster where cmpName='"+jTextField4.getText()+"'");
            if(cmpC!=null)
            ...
            String itemC=inf1.getSingleValue("select itemCode from itemMaster where cmpCode="+cmpCode+" and itemName='"+jTextField5.getText()+"'");
            if(itemC!=null)
            ...
            String stockQ=inf1.getSingleValue("select qty from stockHolding_HPS where itemCode="+itemCode+" and batchNo='"+jTextField3.getText()+"'");
            if(stockQ!=null)
            	stockQty=Integer.parseInt(stockQ);
            /*rs=st.executeQuery("select qty from stockHolding_HPS where itemCode="+itemCode+" and batchNo='"+jTextField3.getText()+"'");
            if(rs.next())
            {
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
        ...
        else        //checking stock of item
        {
            ...
            cmpc=inf.getSingleValue("select qty from stockHolding_HPS where itemCode="+itemCode+" and batchNo='"+jTextField3.getText()+"'");
            if(cmpc!=null)
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            if(flag==0)
            {
                ...
                java.sql.ResultSet rs;*/
                //ZedInterface inf=(ZedInterface)Naming.lookup(getIPaddress.IP());
                //ZedInterfaceForMedical uc1=(ZedInterfaceForMedical)Naming.lookup(getIPaddress.IP());
                //int sno=Integer.parseInt(""+uc1.getSaleNo());
                int sno=uc.getMaxValue("select max(sNo) from saleSmry_ByHPS");
        		sno+=1;
            //java.util.Date dt=new java.util.Date();
            //String entrydate=(dt.getMonth()+1)+"/"+dt.getDate()+"/"+(dt.getYear()+1900);
                String dt1[]=jLabel35.getText().split("/");
            String entrydate=dt1[1]+"/"+dt1[0]+"/"+dt1[2];
               // String entrydate=jLabel35.getText();
            double dispc=0;
            if(uc.isBlank(jTextField6.getText())==false)
            ...
            String s[]=lbl_Label.getText().split("#");
            int patientID=Integer.parseInt(s[0]);
            //java.sql.Statement st=connection.createStatement();
            inf.executeUpdateQuery("insert into saleSmry_ByHPS values("+sno+",'"+entrydate+"',"+dispc+","+taxpc+","+totalitems+","+totalamt+","+disamt+","+taxamt+","+payamt+","+netamt+","+patientID+","+false+")");
            //st.executeUpdate("insert into saleSmry_ByHPS values("+sno+",'"+entrydate+"',"+dispc+","+taxpc+","+totalitems+","+totalamt+","+disamt+","+taxamt+","+payamt+","+netamt+")");
            ...
            for(int r=0;r<dtm1.getRowCount();r++)
            {
                ...
                //st.executeUpdate("insert into saleDtl_ByHPS values("+sno+",'"+coname+"','"+btno+"','"+expdate+"','"+itemnm+"',"+qty+",'"+pkgtype+"',"+mrp+","+amt+")");
                inf.executeUpdateQuery("insert into saleDtl_ByHPS values("+sno+",'"+coname+"','"+btno+"','"+expdate+"','"+itemnm+"',"+qty+",'"+pkgtype+"',"+mrp+","+amt+")");
                int itemcode=0;
                String typeofitem="";
                ...
                /*rs=st.executeQuery("select * from stockHolding_HPS where itemCode="+itemcode+" and batchno='"+btno+"'");
                if(rs.next())
                {
                    qty=rs.getInt("qty")-qty;
                }*/
                if(qty==0)
                    //st.executeUpdate("delete * from stockHolding_HPS where itemCode="+itemcode+" and batchno='"+btno+"'");
                    inf.executeUpdateQuery("delete * from stockHolding_HPS where itemCode="+itemcode+" and batchno='"+btno+"'");
                else
                    //st.executeUpdate("update stockHolding_HPS set qty="+qty+" where itemCode="+itemcode+" and batchno='"+btno+"'");
...
// TODO add your handling code here:
        if((evt.getKeyCode()>=evt.VK_A && evt.getKeyCode()<=evt.VK_Z) || (evt.getKeyCode()>=evt.VK_0 && evt.getKeyCode()<=evt.VK_9) ) 
        {
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            jLabel9.setText(uc.getExpDate(jTextField5.getText(),jTextField3.getText()));
            }
            catch(Exception ex)
			{
     			System.out.println("Error ...."+ex);
        ...
        else if(evt.getKeyCode()==evt.VK_DOWN) 
        {
            ...
            jList3.ensureIndexIsVisible(jList3.getSelectedIndex());
        }
        else if(evt.getKeyCode()==evt.VK_UP) 
        {
            if(jPanel3.isVisible()==false)
   ...
   String Addr,NM;
    ...
    private void printBill()
	{
		try
		{
            ...
            //for(int x=1;x<=80;space+="-",x++);
            //pw.println(space);
                                
                                
            //line=""+cb_mode.getSelectedItem();
            line="ESTIMATE";
			space="";
            rep=(80-line.length())/2;
            ...
            String query="select firmNm from firmProfile";
            //line="SAMRAT MEDICAL HALL";
            line=getDataFromDB(query).toString().toUpperCase();
            pw.print(line);
            
            query="select TINNo from firmProfile";
            String TIN=getDataFromDB(query);
            if(TIN.equals(""))
            {
                rightAlignedText="";
            }
            ...
            {
                rightAlignedText="TIN No.  : "+TIN;
            }
            space="";
            rep=50;
            for(int x=line.length();x<rep;space+=" ",x++);
            pw.print(space);
            line=rightAlignedText;
            pw.println(line);
            rightAlignedText="";
            
            query="select ads from firmProfile";
            //line="Sindhora Bazar, Varanasi";
            line=getDataFromDB(query);
            pw.print(line);
            
            query="select regNo from firmProfile";
            String reg=getDataFromDB(query);
            if(reg.equals(""))
            {
                rightAlignedText="";
            }
            ...
            pw.println(line);
            rightAlignedText="";
           /* line="Name    : "+tf_custName.getText().toUpperCase();
            pw.print(line);*/
            ...
            for(int x=1;x<=rep;space+=" ",x++);
            pw.print(space);
            line="Bill No. : "+jLabel33.getText();
            pw.println(line);
            
            /*line="Address : "+tf_custAddr.getText().toUpperCase();
            pw.print(line);*/
            
            ////////////////////////code Updated Here//////////
            line="Customer Name:"+NM;
            pw.print(line);
          ...
          /*  space="";
            ...
            for(int x=1;x<=rep;space+=" ",x++);
            pw.print(space);*/
            //line="Date : "+cb_date.getSelectedItem()+"-"+cb_month.getSelectedItem()+"-"+cb_year.getSelectedItem();
            line="Date     : "+jLabel35.getText();
            pw.println(line);
            ...
            space="";
            for(int x=1;x<=80;space+="_",x++);
            pw.println(space);
            
            line="";
           /* if(cb_mode.getSelectedIndex()==0)
            {
            	line=" QTY.    PARTICULARS                                                    AMOUNT";
            }*/
            //else
            {
            	line=" QTY.    PARTICULARS                   BATCH      EXP.         MRP      AMOUNT";
            }
            pw.println(line);
            ...
            for(int r=0;r<dtm1.getRowCount();r++)
            {
            	...
            	//item name
            	line="     "+jTable1.getValueAt(r,4).toString().toUpperCase()+" "+jTable1.getValueAt(r,6).toString().toUpperCase();
	            pw.print(line);
            	...
            	/*else
            	{
    		        ...
    		        rep=(58-line.length());            
        		    for(int x=1;x<=rep;space+=" ",x++);
    		        line=""+tab.getValueAt(r,6);
            		rep=(15-line.length());            
        		    for(int x=1;x<=rep;space+=" ",x++);
	            	pw.println(space+line);            		
            	}*/            	
            }
			for(int r=1;r<=(10-dtm1.getRowCount());r++)
			{
				pw.println();
			}
            space="";
			...
			line=" Total No. Of Items : " +dtm1.getRowCount();
            pw.print(line);

       		space="";
	        rep=(41-line.length());            
   		    for(int x=1;x<=rep;space+=" ",x++);
            line="TOTAL AMOUNT (In Rs.) : ";
            pw.print(space+line);

			space="";
			line=jLabel39.getText();
			rep=(12-line.length());            
   		    for(int x=1;x<=rep;space+=" ",x++);
            pw.println(space+line);
            ...
            line="E&OE Subject to VARANASI jurisdiction.";
            pw.println(line);
            System.out.println("01");
            query="select DLNo from firmProfile";
            String DL=getDataFromDB(query);
            if(DL.equals(""))
            {
                line="";
            }
            ...
            System.out.println("2");
            //line="DL No. : VNS-113/20/2008, VNS-113/21/2008";
            pw.print(line);
		...
		{
			System.out.println("Error in printBill method of MakingSale class...."+ex);
		}
	}
    private String getDataFromDB(String query)
    {
        String data="";
        try
        {
