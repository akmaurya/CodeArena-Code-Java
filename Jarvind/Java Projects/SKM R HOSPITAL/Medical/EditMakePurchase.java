 ...
 * @author  Deep Pal
 */
public class EditMakePurchase extends javax.swing.JInternalFrame {
    
    /** Creates new form MakePurchase */
    public EditMakePurchase(javax.swing.JDesktopPane jDesktopPane1,int pNo) {
        initComponents();
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            String qry="select partyNm,partyAds,bookNo,srNo,entrydt,totalitems,totalAmt,roAmt,netAmt,wholeTypeOfDrug,wholeTypeOfPay from purchaseSmry where pNo="+pNo;
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("Edit Purchase Bill :: Wizard 2/2");
        ...
        jButton1.setBounds(430, 440, 190, 30);
        ...
        jButton4.setBounds(670, 440, 120, 30);
        ...
        jButton7.setBounds(760, 0, 23, 20);
...
//	ZedInterfaceForMedical inf1=(ZedInterfaceForMedical)Naming.lookup(getIPaddress.IP());
	try
		{
		//	ZedInterface inf=null;
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            //-----------------------
                ...
                inf.executeUpdateQuery("delete * from purchasePaymentSmry where pNo="+pNo);
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error with connection with server  rmi://127.0.0.1/ExtendZedServer...");
            ...
            //taking max edit no
            int editNo=inf.getMaxValue(query);
            editNo+=1;
            //save data 1
            java.util.Date dt=new java.util.Date();
            String dateAndTime=(dt.getMonth()+1)+"/"+dt.getDate()+"/"+(dt.getYear()+1900)+" "+dt.getHours()+":"+dt.getMinutes()+":"+dt.getSeconds();
            query="insert into logFileForUpdate values('"+dateAndTime+"','EditPurchaseBill',"+editNo+")";
            inf.executeUpdateQuery(query);
            //getting data from db
            query="select * from purchaseSmry where pNo="+pNo+"";
            str=ut.returnResultSet(query,"EditMakePurchaseWindowSmry","");
            //save data 2
            query="insert into purchaseSmryEdit values('"+str[0][0]+"','"+str[0][1]+"','"+str[0][2]+"','"+str[0][3]+"','"+str[0][4]+"','"+str[0][5]+"','"+str[0][6]+"','"+str[0][7]+"','"+str[0][8]+"','"+str[0][9]+"',"+editNo+",'"+str[0][10]+"','"+str[0][11]+"')";
            ut.saveOrUpdateResultSet(query);
            //counting value of purDtl with existing 
            query="select * from purchaseDtl where pNo="+pNo+"";
            int count=ut.countingValue(query);
            //taking data from DB
            query="select * from purchaseDtl where pNo="+pNo+"";
            str=ut.returnResultSet(query,"editMakePurchaseWindowDtl",""+count);
            //System.out.println("len="+str.length);
            //save data 3
            for(int i=0;i<str.length;i++)
            {
                query="insert into purchaseDtlEdit values("+editNo+",'"+str[i][1]+"','"+str[i][2]+"','"+str[i][3]+"','"+str[i][4]+"','"+str[i][5]+"','"+str[i][6]+"','"+str[i][7]+"','"+str[i][8]+"','"+str[i][9]+"','"+str[i][10]+"','"+str[i][11]+"','"+str[i][12]+"')";
                ut.saveOrUpdateResultSet(query);
            }
        ...
        {
            System.out.println("Error in takePrevoiusDataAndSaveIntoDB method into EditMakePurchase class..."+e);
        }
    }
    ...
    private javax.swing.JTextField jLabel20;
