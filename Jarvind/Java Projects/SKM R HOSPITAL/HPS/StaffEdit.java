...
public class StaffEdit implements ActionListener,KeyListener,MouseListener
{
	...
	JButton save,refresh,close,delete,but_deg,butt_calender;
...
try
		...
		close=new JButton("Close",new ImageIcon("icons\\exit.gif"));
                close.setMnemonic('c');
                delete=new JButton("Delete",new ImageIcon("icons\\dustbeen.gif"));
		...
		close.setBounds(375, 450, 110, 33);
		close.setToolTipText("Close");
		cn.add(close);

                delete.setBounds(255,450,110,33);
                cn.add(delete);
...
try
        ...
        private boolean delButton()
	{
		...
		{
                    int x=10;
			String nam=jt.getValueAt(jt.getSelectedRow(), 1).toString();
                        String deg=jt.getValueAt(jt.getSelectedRow(), 6).toString();
                        if(deg.equalsIgnoreCase("Doctor"))
                        x=JOptionPane.showConfirmDialog(null, "Do you want to delete the staff(doctor) Mr. "+nam, null, JOptionPane.YES_NO_OPTION);
                        else
                            x=JOptionPane.showConfirmDialog(null, "Do you want to delete the staff Mr."+nam, null, JOptionPane.YES_NO_OPTION);
                        if(x==JOptionPane.YES_OPTION)
                        {
                            String qry="delete * from staff_Details where s_Name='"+jt.getValueAt(jt.getSelectedRow(), 1).toString()+"'and s_Age="+Integer.parseInt(jt.getValueAt(jt.getSelectedRow(), 2).toString())+"and s_Contact='"+jt.getValueAt(jt.getSelectedRow(), 5).toString()+"'and s_Designation='"+jt.getValueAt(jt.getSelectedRow(), 6).toString()+"'";
                            System.out.println();
                            inf.executeUpdateQuery(qry);
                            qry="delete * from drDetail where drName='"+jt.getValueAt(jt.getSelectedRow(), 1).toString()+"'";
                            System.out.println();
                            inf.executeUpdateQuery(qry);
                            JOptionPane.showMessageDialog(jtf,"Staff deleted Successfully ","Congratulation !!!",JOptionPane.INFORMATION_MESSAGE);
