...
public class EditFeeDetail extends JFrame implements ActionListener,KeyListener
{
	JInternalFrame editfeefrm;
	JPanel pn;
	Date dt1,dt2;
	JTextField tf_patientID,tf_extrachr,tf_charge,tf_charge1,lbl_YAG,lbl_consfee;
	...
	String qry,setip,currentdate,setdate,add_name,st="",getID;
	int flag,MFLAG,dateflag,TOTFlag=0,pid=0,chkCORRECTID=0,Flag_lessthan=0,FlagZero=0,add=0,getID1=0;
	double addfee=0,addYAGfee=0,addexcharge=0,addMorecharges=0;
	Date dt;
	ZedInterface inf=null;
	public EditFeeDetail(JDesktopPane dp1,int ID)
	{
		getID=""+ID;
	...
	//	flag=FLAG;
		...
		editfeefrm=new JInternalFrame("EDIT FEE DETAIL WINDOW",true,true,false,true);
		editfeefrm.setDefaultCloseOperation(DO_NOTHING_ON_CLOSE);
		Container cn=editfeefrm.getContentPane();
		cn.setLayout(null);
		Color c1=new Color(60,220,220);
 		cn.setBackground(c1);
		editfeefrm.setBounds(100,50,400,520);
	...
	//	tf_extrachr=new JTextField(javax.swing.JTextField.RIGHT);
		...
		bt_SAVE=new JButton("Edit",new ImageIcon("icons\\save.gif"));
		bt_SAVE.setMnemonic('i');
		bt_SAVE.setBounds(25,360,150,30);
		bt_SAVE.setToolTipText("Edit");
...
//		System.out.println("currentdate "+currentdate);
	...
	public void keyPressed(KeyEvent ke)
	{
		...
		if(ke.getSource()==chk_YAG && ke.getKeyCode()==ke.VK_ENTER)
		{
					...
					//	long exp30=oneDayValue*30;
						...
						//System.out.println("day "+day);
							...
							if(daydiff>30)
							{
								...
								lbl_lastVisitdt.setText(dbdate);
							...
							qry="Select cons_fee,YAG_techNM,YAG_charge,extra_charge,additional_chargeNM,additional_charge,total_amount from receivingPatient_FeeDetail where patientID="+pid+" Order By visit_dt desc";
							System.out.println("qry.."+qry);
							String arr[][]=inf.getResultSetAs_2D_Array(qry,7);
							if(arr!=null && arr.length>0)
							{
								for(int i=0;i<arr.length;i++)
								{
								//	int c=0;
									//System.out.println("0.."+arr[i][c]);
									lbl_consfee.setText(""+(new java.math.BigDecimal(Double.valueOf(arr[i][0])).setScale(2,java.math.BigDecimal.ROUND_HALF_UP)));
									chk_fee.setSelected(true);
									if(!(arr[i][1]).equals(""))
									{
										cb_YAGtech.setSelectedItem(arr[i][1]);
										chk_YAG.setSelected(true);
									}
									else
									{
										cb_YAGtech.setSelectedItem("SELECT");
									}
									//System.out.println("1.."+arr[i][++c]);
									if(!(arr[i][2]).equals(""))
									{
										lbl_YAG.setText(""+(new java.math.BigDecimal(Double.valueOf(arr[i][2])).setScale(2,java.math.BigDecimal.ROUND_HALF_UP)));
									}
									
									//System.out.println("2.."+arr[i][++c]);
									if(Integer.parseInt(arr[i][3])!=0)
									{
										tf_charge.setText(""+(new java.math.BigDecimal(Double.valueOf(arr[i][3])).setScale(2,java.math.BigDecimal.ROUND_HALF_UP)));
										chk_charge.setSelected(true);
									}
									
								//	System.out.println("3.."+arr[i][++c]);
									if(!(arr[i][4]).equals(""))
									{
										tf_charge1.setText(arr[i][4]);
									}
									
									//System.out.println("4.."+arr[i][++c]);
									if(Integer.parseInt(arr[i][5])!=0)
									{
											tf_extrachr.setText(""+(new java.math.BigDecimal(Double.valueOf(arr[i][5])).setScale(2,java.math.BigDecimal.ROUND_HALF_UP)));
											chk_extracharges.setSelected(true);
									}
									
								//	System.out.println("5.."+arr[i][++c]);
									if(Integer.parseInt(arr[i][6])!=0)
									{
										double totfrmdb=Double.valueOf(arr[i][6]);
										System.out.println("totfrmdb..."+totfrmdb);
									}
									//System.out.println("6.."+arr[i][++c]);
									
								}
							}
							else
							{
								JOptionPane.showMessageDialog(this,"No Data Available In PatientFeeDetail Table","Message",JOptionPane.ERROR_MESSAGE);		
							}
...
//	}
	private void GetTotalFun()
	{
			...
			else
			{
				...
				try
				{
					addfee=Double.valueOf(fee);// use here Double.valueOf(fee) n collect vale in double type variable
				tot=tot+addfee;
				System.out.println("totfee..."+tot);
				}
				catch(Exception ex)
				{
				...
				if(chk_YAG.isSelected()==true)
				{
					...
					else
					{
						try
						{
							addYAGfee=Double.valueOf(YAGfee);
							tot=tot+addYAGfee;
						}
						catch(Exception ex)
						{
				...
				if(chk_extracharges.isSelected()==true)
				{
					...
					else
					{
						try
						{
							addexcharge=Double.valueOf(excharge);
							tot=tot+addexcharge;
						}
						catch(Exception ex)
						{
				...
				if(chk_charge.isSelected()==true)
				{
					...
					else
					{
						try
						{
							addMorecharges=Double.valueOf(additional);
				...
				lbl_Amt.setText(""+(new java.math.BigDecimal(tot).setScale(2,java.math.BigDecimal.ROUND_HALF_UP)));
				TOTFlag=1;
			}
	...
	private void SavePatientFeeDetailFun()
	{
		...
		double consfee=addfee;
		double YAGfee=addYAGfee;
		double extracharge=addexcharge;
		double moreextracharge=addMorecharges;
			//	String add_chargeName=tf_charge.getText();
			//	int add_charge=tf_charge1.getText();
		String gettot=lbl_Amt.getText();
		double TOTAL=Double.valueOf(lbl_Amt.getText());//use double.valueOf(gettot) collect value in double type variable
...
try
					...
					qry="Select patientID,throughPaymentCome from patient_All_Invoice where patientID="+Integer.parseInt(tf_patientID.getText())+" Order By receiptDt desc";
					System.out.println("qry...."+qry);
					String arr[][]=inf.getResultSetAs_2D_Array(qry,2);
					if(arr!=null && arr.length>0)
					{
						System.out.println("0..."+arr[0][0]);
						System.out.println("1..."+arr[0][1]);
						if(arr[0][1].equals("Through OPD"))
						{
							JOptionPane.showMessageDialog(this,"Database can't be Updated","Message",JOptionPane.INFORMATION_MESSAGE);
					...
					{
						qry="update receivingPatient_FeeDetail set YAG_techNM='"+cb_YAGtech.getSelectedItem()+"',YAG_charge="+YAGfee+",extra_charge="+addexcharge+",additional_chargeNM='"+tf_charge.getText()+"',additional_charge="+addMorecharges+",total_amount="+TOTAL+" where patientID="+Integer.parseInt(tf_patientID.getText())+" and visit_dt=#"+lbl_lastVisitdt.getText()+"#";
						System.out.println("qry...."+qry);
