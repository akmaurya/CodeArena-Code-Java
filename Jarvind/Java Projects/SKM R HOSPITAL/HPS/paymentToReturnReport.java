...
public class paymentToReturnReport implements ActionListener,KeyListener
{	
...
//	ZedInterface zedint;
	...
	public paymentToReturnReport(JDesktopPane desk)
	{	
		jtf=new JInternalFrame("Payment Return Details",false,true,false,true);
		...
		String head[]={"S.No.","Date","ID","Purpose To Pay","Amount","Paid To"};
		...
		column=jt.getColumn("Purpose To Pay");
	...
	private void showButton()
	{
		...
		{	
			...
			{	
				...
				String str1[]=inf.getResultSetAs_1D_Array("select date from payment_ToReturn order by date ");
				hdate1=str1[0];
				str=hdate1.split("-");
				year=str[0];
				month=str[1];
			...
			sno=0;
			dtm.setRowCount(0);
			String pid[]=inf.getResultSetAs_1D_Array("select ID from payment_ToReturn where date>=#"+hdate1+"# and date<=#"+hdate2+"# order by date");
			...
			for(int l=0;l<pid.length;l++)
			{
				if(pid[l]!=null)
				{
					...
					arr=inf.getResultSetAs_2D_Array("select date,purposeToPay,amt,paidTo from payment_ToReturn where ID='"+(pid[l])+"'",4);
					if(arr!=null)
					{
