 ...
 */
public class ProvideSpectsToCustomer extends javax.swing.JInternalFrame {
    
    /** Creates new form ProvideSpectsToPatient */
    public ProvideSpectsToCustomer(JDesktopPane jDesktopPane1) {
...
try
					...
					int max=inf.getMaxValue("select max(srno) from customer_SpectsProvidation");
					max+=1;
        			...
        			int convrt=Integer.parseInt(getLastId);
        			System.out.println("convrt  "+convrt);
        			if(getLastId.equals("0"))
        			{
        					Cid="C"+1;
        					System.out.println("cid  "+Cid);
        		...
        		System.out.println("Cid  "+Cid);
        			...
        			//----amount related code--------
        			double frameAmt=0,glassAmt=0,extraAmt=0,paidAmt=0;
        		//	int quantity=0;
        			...
        			paidAmt=Double.valueOf(ppp.jTextField4.getText());
				...
				//	String query="insert into patient_SpectsProvidation values("+max+","+patientid+",#"+curDt+"#,"+catcodeF+","+itmcodeF+","+cmpcodeF+",'"+size+"',"+frameAmt+","+catcodeG+","+itmcodeG+","+cmpcodeG+",'"+lens+"',"+glassAmt+","+extraAmt+",#"+givenDt+"#,"+totalAmt+","+dueAmt+","+false+")";
					String query="insert into customer_SpectsProvidation values("+max+",'"+Cid+"','"+jTextField3.getText().trim()+"',#"+curDt+"#,"+catcodeF+","+itmcodeF+","+cmpcodeF+",'"+size+"',"+frameAmt+","+catcodeG+","+itmcodeG+","+cmpcodeG+",'"+lens+"',"+glassAmt+","+extraAmt+",#"+givenDt+"#,"+totalAmt+","+dueAmt+","+false+")";
					...
					int maxNxt=inf.getMaxValue("select max(receiptNo) from patient_All_Invoice where status=0");
					maxNxt+=1;
					java.util.Date dt1=new java.util.Date();
					String curDt1=(dt1.getMonth()+1)+"/"+dt1.getDate()+"/"+(dt1.getYear()+1900);
					query="insert into patient_All_Invoice values("+maxNxt+",#"+curDt1+"#,"+(-3)+",'Through Spects(Direct Sale To Customer)',"+paidAmt+",0)";
				...
				/*	String qry="Select qty from purchaseDtlOfSpects where catCode="+catcodeF+" and itmCode="+itmcodeF+" and cmpCode="+cmpcodeF+"";
					...
					int quanty=0;
					qry="Select qty from spectsStockHolding where cmpCode="+cmpcodeF+" and itmCode="+itmcodeF+" and size_lense_No='"+size+"'";
     ...
     class printReceipt extends JFrame 
   {
	...
	int currentrec=0,startno=0,endno=0;
        ...
        public void drawPage(Graphics2D g2)
        {
            ...
            Font f9=new Font("Arial",Font.PLAIN,8);
            FontMetrics fm;
...
try
		            ...
		            temp="Cust.ID";
		            layout=new TextLayout(temp,f3,context);
        		    g2.drawString(temp,(270-fm.stringWidth(temp) -fm.stringWidth(""+jLabel10.getText().length()))/2-10,top);
		            //opdNo
		            g2.setFont(f7);
		            fm=g2.getFontMetrics();
		            ...
		            fm=g2.getFontMetrics();
		            temp=""+name;
		            System.out.println("Customer name   "+temp);
		            layout=new TextLayout(temp,f7,context);
        		    ...
        		    temp=" Age/Sex :  ";
		            layout=new TextLayout(temp,f3,context);
        		    g2.drawString(temp,(260-fm.stringWidth(temp) -fm.stringWidth("       "+"        ")),top);
