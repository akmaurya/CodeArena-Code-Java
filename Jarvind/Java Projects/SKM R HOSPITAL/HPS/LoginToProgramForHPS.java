...
import java.io.*;
import java.rmi.*;
class LoginToProgramForHPS  extends JFrame implements ActionListener,KeyListener
{
	Container cn;
	JDialog dg;
	JLabel notes;
...
catch(Exception e)
{
	...
	catch(Exception ex)
	{
		System.out.println("Error at the time of connection with server ip-rmi//127.0.0.1/ExtendZedServer  ..."+ex);
			...
			String[][] rs=inf.getResultSetAs_2D_Array("select uId,uPws,uLevel from loginMasterOfHPS",3);
				...
				if(str1.equals(log.getText()))
				{
					...
					if(str2.equals(pws.getText()))
					{
						...
						{	
							...
							inf.executeUpdateQuery("insert into DateOfOpeningAppln values('"+str1+"','"+dateTime+"','HOSPITAL')");
							System.out.println("1...");
							dg.dispose();
							DeskTop mbFrm=new DeskTop(log.getText(),cbLog.getSelectedIndex());
		...
		{
			System.out.println("Error...123."+s);
		}
