import java.rmi.*;
import java.rmi.registry.*;
import java.awt.*;
import javax.swing.*;
import javax.swing.table.*;
import java.awt.event.*;
import java.sql.*;
public class StaffDetailsById implements ActionListener,KeyListener
{
	JLabel jl1;
	JButton show,close;
	JTextField jtex;
	JTable jt;
	DefaultTableModel dtm;
	TableColumn column;
	ZedInterface inf;
	JInternalFrame jtf;
	
	int count,sno;
	String arr[][],arr1[],id;
	public StaffDetailsById(JDesktopPane desk)
	{
		jtf=new JInternalFrame("Staff:Edit Log Report",false,true,false,true);
		Container cn=jtf.getContentPane();
		cn.setLayout(null);
		jtex=new JTextField("",JTextField.CENTER);
		jl1=new JLabel("Enter Staff ID :");
		jl1.setFont(new Font("Microsoft Sans Serif",Font.BOLD,13));

		String head[]={"S.N.","Name","Address","Contact No","Designation","Salary","Salary Basis","Edit Date And Time"};
		dtm=new DefaultTableModel(head,0);
		jt=new JTable(dtm);
		jt.setFont(new Font("Microsoft Sans Serif",Font.PLAIN,14));
		
		jt.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);

		DefaultTableCellRenderer CRLeft = new DefaultTableCellRenderer();
		DefaultTableCellRenderer CRRight = new DefaultTableCellRenderer();
		DefaultTableCellRenderer CRCenter = new DefaultTableCellRenderer();
		
		CRLeft.setHorizontalAlignment(JLabel.LEFT);
	       	CRRight.setHorizontalAlignment(JLabel.RIGHT);
       		CRCenter.setHorizontalAlignment(JLabel.CENTER);
		
		column=jt.getColumn("S.N.");
      		column.setCellRenderer(CRCenter);
       		column.setPreferredWidth(40);

      		column=jt.getColumn("Name");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(120);
		
		column=jt.getColumn("Address");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(150);
		
		column=jt.getColumn("Contact No");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(120);
		
		column=jt.getColumn("Designation");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(80);

		column=jt.getColumn("Salary");
      		column.setCellRenderer(CRRight);
       		column.setPreferredWidth(80);

		column=jt.getColumn("Salary Basis");
      		column.setCellRenderer(CRCenter);
       		column.setPreferredWidth(90);

                column=jt.getColumn("Edit Date And Time");
      		column.setCellRenderer(CRCenter);
       		column.setPreferredWidth(200);
		
		JScrollPane jsp=new JScrollPane(jt,JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		show=new JButton("SHOW",new ImageIcon("icons\\save1.gif"));
		close=new JButton("CLOSE",new ImageIcon("icons\\exit.gif"));
		jl1.setBounds(10,25,130,30);
		jtex.setBounds(130,28,100,28);
		show.setBounds(450,26,110,30);
		jtex.setFont(new Font("Microsoft Sans Serif",Font.PLAIN,15));
		jsp.setBounds(10,70,670,300);
		close.setBounds(570,26,110,30);
		
		cn.add(jl1);
		cn.add(jtex);
		cn.add(close);
		cn.add(show);
		cn.add(jsp);
		
		close.setMnemonic('c');
		close.setToolTipText("Close");	
		
		close.addActionListener(this);
		show.addActionListener(this);
		close.addKeyListener(this);
		show.addKeyListener(this);

		jtex.addKeyListener(this);
		jtf.setBackground(new Color(60,220,220));
		jtf.reshape(100,100,700,420);
		
		desk.add(jtf);
		jtf.setVisible(true);
	/*	try
		{
		//	inf=(inferface)Naming.lookup(getIPaddress.IP());
		
			jtex.requestFocus();
		}
		catch(Exception e)
		{
			System.out.println("Error "+e);
		}*/
		
		try
							{
								inf=(ZedInterface)Naming.lookup(getIPaddress.IP());
								jtex.requestFocus();
				
							}
							catch(Exception e)
							{
									System.out.println("Error at the time of connection with server "+getIPaddress.IP()+"..."+e);
									getIPaddress.changeIP();
									try
									{
										inf=(ZedInterface)Naming.lookup(getIPaddress.IP());
										jtex.requestFocus();	
										
									}
									catch(Exception ex)
									{
										System.out.println("Error at the time of connection with server ip-rmi//127.0.0.1/ExtendZedServer  ..."+ex);
									}
							}	
		
		
	}
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource()==close)
		{
			jtf.dispose();
		}
		if(ae.getSource()==show)
		{
			showButton();
		}
	}
	public void keyPressed(KeyEvent ke)
	{	
		if(ke.getSource()==jtex && ke.getKeyCode()==KeyEvent.VK_ENTER)
		{
			show.requestFocus();
		}
		if(ke.getSource()==show && ke.getKeyCode()==KeyEvent.VK_ENTER)
		{
			showButton();
		}
		if(ke.getSource()==close && ke.getKeyCode()==KeyEvent.VK_ENTER)
		{
			jtf.dispose();
		}
	}
	public void keyReleased(KeyEvent ke)
	{
	}
	public void keyTyped(KeyEvent ke)
	{
	}
	private void showButton()
	{
		try
		{	
			int code=0;
			boolean flag=true;
			id=jtex.getText();
			try
			{
				code=Integer.parseInt(id);
				String staffid[]=inf.getResultSetAs_1D_Array("select s_Code from staff_Details");
				for(int i=0;i<staffid.length;i++)
				{
					if(Integer.parseInt(staffid[i])==code)
						break;
					if(i==staffid.length-1)
					{	
						JOptionPane.showMessageDialog(jtex,"Please Input Correct ID","Error",JOptionPane.ERROR_MESSAGE);
						jtex.requestFocus();
						flag=false;
					}
				}	
			}
			catch(Exception e)
			{	
				JOptionPane.showMessageDialog(jtex,"Please Input Correct ID(Numbers Only)","Error",JOptionPane.ERROR_MESSAGE);
				jtex.requestFocus();
				jtex.setText("");
				flag=false;		
			}
			if(flag)
			{
				arr1=new String[8];
				count=inf.getCountValue("select count(s_Code) from staff_Details");
				arr=new String[count][7];
				if(count>0)
				{
					arr=inf.getResultSetAs_2D_Array("select s_Name,s_Ads,s_Contact,s_Designation,s_Salary,s_SalaryBasis,s_DOJ from staff_Details where s_Code="+code,7);
					
					if(arr!=null)
					{
						dtm.setRowCount(0);
						sno=0;
						arr1[0]=""+(++sno);
						arr1[1]=arr[0][0];					
						arr1[2]=arr[0][1];
						arr1[3]=arr[0][2];
						arr1[4]=arr[0][3];
						arr1[5]=arr[0][4];
						arr1[6]=arr[0][5];
                                                String itm[]=arr[0][6].split(" ");
                                                        String itm1[]=itm[0].split("-");
                                                        String tm[]=itm[1].split(":");
                                                        int hour=Integer.parseInt(tm[0]);
                                                        String tfor="AM";
                                                        if(hour>=12)
                                                        {
                                                            hour=hour-12;
                                                            tfor="PM";
                                                        }
                                                        String time=hour+":"+tm[1]+":"+tm[2];
                                                        arr1[7]=itm1[2]+"/"+itm1[1]+"/"+itm1[0]+" "+time+" "+tfor;

						dtm.addRow(arr1);
					}
					arr=inf.getResultSetAs_2D_Array("select s_Name,s_Ads,s_Contact,s_Designation,s_Salary,s_SalaryBasis,editNo from staff_DetailsEdit where s_Code="+code,7);
					if(arr!=null)
					{
                                            String temp="EditStaff";
						for(int i=0;i<arr.length;i++)
						{
							arr1[0]=""+(++sno);
							arr1[1]=arr[i][0];						
							arr1[2]=arr[i][1];
							arr1[3]=arr[i][2];
							arr1[4]=arr[i][3];
							arr1[5]=arr[i][4];
							arr1[6]=arr[i][5];
                                                        String qry="Select editDtTime from logFileForUpdate where editNo="+Integer.parseInt(arr[i][6])+" and formNm='"+temp+"'";
                                                        String edate=inf.getSingleValue(qry);
                                                        System.out.println(edate);
                                                        String itm[]=edate.split(" ");
                                                        String itm1[]=itm[0].split("-");
                                                        String tm[]=itm[1].split(":");
                                                        int hour=Integer.parseInt(tm[0]);
                                                        String tfor="AM";
                                                        if(hour>=12)
                                                        {
                                                            hour=hour-12;
                                                            tfor="PM";
                                                        }
                                                        String time=hour+":"+tm[1]+":"+tm[2];
                                                        arr1[7]=itm1[2]+"/"+itm1[1]+"/"+itm1[0]+" "+time+" "+tfor;
							dtm.addRow(arr1);
						}
					}			
				}
				else
				{
					JOptionPane.showMessageDialog(jt,"No Record Exists","Message",JOptionPane.INFORMATION_MESSAGE);	
				}
			}
		}
		catch(Exception e)
		{
			System.out.println("Error"+e);
		}
	}	
}