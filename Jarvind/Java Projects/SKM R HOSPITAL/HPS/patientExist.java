...
class patientExist extends JFrame implements ActionListener,KeyListener
{
	JInternalFrame existingfrm;
	JDesktopPane Feedp;
	...
	JButton bt1,refresh,btallergy,btcurrentmedi,SAVE,CLOSE,bt_SaveRef,bt_CloseRef,bt_SaveAllergy,bt_CloseAllergy,bt_addDt,btn_refresh;
...
;
	...
	String setip,qry,str="",str1="";
	ZedInterface inf;
	Date dt2;
	
	public patientExist(JDesktopPane dp1)
	{
		Feedp=dp1;
		vc=new Vector();
		vc1=new Vector();
	...
	//	mydialog mydg=new mydialog();
		...
		existingfrm=new JInternalFrame("EXISTING PATIENT WINDOW",true,true,false,true);
		Container cn=existingfrm.getContentPane();
		cn.setLayout(null);
	//	Color c1=new Color(60,220,220);
 		cn.setBackground(c1);
		existingfrm.setBounds(100,50,700,600);
		setTitle("EXISTING PATIENT WINDOW");

		lb=new JLabel("DATE",JLabel.LEFT);
		lb.setOpaque(false);
		...
		lbdate=new JLabel(condt+"/"+mon1+"/"+year1,JLabel.CENTER);
...
public void actionPerformed(ActionEvent ae)
{
	...
	else if(ae.getSource()==SAVE)
	{
		...
		else
		{
			...
			else
			{
					...
					{
						System.out.println("entering....1111111111 ");
						checkPreviousEntry();
					}
			}
	...
	else if(ae.getSource()==cbdrnm)
	{
		...
		catch(Exception e)
		{
			...
			{
				System.out.println("ERROR ...Bt there is no problem here"+ex);
			}
		}
	}
	...
	public void keyPressed(KeyEvent ke)
	{
		...
		else if(ke.getSource()==pidT && ke.getKeyCode()==ke.VK_ENTER)
		{
			...
			else
			{
				...
				int chkCORRECTID=0,checkID=0,sendID=0;
				String getid="";
		...
		catch(Exception e)
		{
				...
				catch(Exception ex)
				{
					...
					if(loadID!=null)
					{
						...
						{
								chkCORRECTID=1;
								checkID=Integer.parseInt(getid);
						}
					...
					if(chkCORRECTID==1)
					{
						System.out.println("errr...11");	
						FetchExistingPatientInfo(checkID,chkCORRECTID);	
					}
				}
				catch(Exception ex)	
				{
	...
	{}
	private void FetchExistingPatientInfo(int checkID,int chkCORRECTID)
	{
			int chkCORRECTID1=0,chkCORRECTID3=0;
		//	String getid=pidT.getText()	;
			int sendID=checkID;
			chkCORRECTID3=chkCORRECTID;
			System.out.println("chkCORRECTID3....."+chkCORRECTID3);
...
try
				...
				//	}
					if(chkCORRECTID3==1)
					{
						...
						if(loadID1!=null)
						{
							if((loadprofile[0][0]).equals(loadID1))
								chkCORRECTID1=1;
						...
						if(chkCORRECTID1==1)
						{
							qry="Select allergy,current_medi from history_Patient where patient_id="+sendID+" Order by history_date desc";
							System.out.println("qry...."+qry);
							String loadHistory[][]=inf.getResultSetAs_2D_Array(qry,2);
							if(loadHistory==null)
							{
							//	System.out.println("ERRRRRRRRR...111");
								JOptionPane.showMessageDialog(this,"Record Not Available","Acknowledgement",JOptionPane.ERROR_MESSAGE);
							...
							//	System.out.println("allergy "+loadHistory[0][0]);
								String allergy=loadHistory[0][0];
							//	System.out.println("allergy "+loadHistory[0][1]);
								String currentmedi=loadHistory[0][1];
		...
		{
			System.out.println("ERROR...IN FetchExistingPatientInfo"+ex);
		}
...
try
							...
							qry="Update patient_Profile set drCatCode="+code+",drCode="+dcode+",age='"+ageT.getText()+"',gender='"+rbSelected+"',dob='"+dobT.getText()+"',address='"+addressT.getText()+"',phone_no='"+phoneT.getText()+"',gurdian='"+gurdianT.getText()+"',refered_by='"+(""+cb1.getSelectedItem())+"',city='"+cityT.getText()+"',pincode='"+pincodeT.getText()+"',state='"+stateT.getText()+"' where patientid="+sendID+" And name='"+nameT.getText()+"'";
				//	System.out.println("qry "+qry);	
				//	qry="Update patient_Profile set drCatCode="+code+",drCode="+dcode+",age='"+st2+"',gender='"+rbSelected+"',dob='"+st3+"',address='"+st4+"',phone_no='"+st5+"',gurdian='"+st6+"',refered_by='"+st7+"',city='"+st8+"',pincode='"+st9+"',state='"+st10+"',status="+true+" where patientid="+sendID+" And name='"+st1+"'";
						//	System.out.println("qry "+qry);
						...
						//	System.out.println("qry "+qry);
				//	qry="insert into history_Patient values("+sendID+",'"+stp1+"','"+stp2+"','"+stp3+"','"+stp4+"','"+stp5+"','"+stp6+"','"+stp7+"','"+stp8+"',#"+currentdate+"#,"+true+")";
							System.out.println("qry "+qry);
						...
						//	System.out.println("Y1 "+Y1);
							...
							if(successful2==true && successful1==true)
							{
								FLAG=2;
							//	String getdt=lbdate.getText();
								String Hold_dt1=getdt;
								System.out.println("Hold_dt1..."+Hold_dt1);
								FeeDetails fee=new FeeDetails(Feedp,sendID,FLAG,Hold_dt1);
				...
				{
							System.out.println("ERROR..IN SavePatientInfo"+ex);
				}
		}
	}
...
try
			...
			{
				System.out.println("Error..IN AddNewReferenceFun"+ex);
			}
	}
...
try
	...
	private void checkPreviousEntry()
	{
		
			int sendID=Integer.parseInt(pidT.getText());
		...
		try
						...
						qry="Select patient_id from history_Patient where patient_id="+sendID+" And status="+true+" Order By history_date desc";
						System.out.println("qry "+qry);
						String loadID1=inf.getSingleValue(qry);
						if(loadID1!=null)
						{
							JOptionPane.showMessageDialog(this,"Already Registered Today","Message",JOptionPane.ERROR_MESSAGE);						
							refreshAll();
							pidT.requestFocus();
						}			
						else
			...
			{
				System.out.println("ERROR. ..IN checkPreviousEntry Method"+ex);
			}
