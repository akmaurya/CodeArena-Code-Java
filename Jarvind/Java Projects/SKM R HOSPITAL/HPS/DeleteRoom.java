...
public class DeleteRoom extends JInternalFrame implements ActionListener,KeyListener,MouseListener
{
	JButton btn1,btn2;
	JComboBox cb1;
	Container cn;
	...
	public DeleteRoom(JDesktopPane dp)
	{
		frm=new JInternalFrame("Delete Room",false,true,false,true);
     ...
     String str[]=inf.getResultSetAs_1D_Array("select wardCatName from wardCategory");
	...
	catch(Exception ex)
       {
        ...
        dtm=new javax.swing.table.DefaultTableModel(heading,0);
         ...
         jsp.setBounds(20,70, 380,100);
         cn.add(jsp);
        addingValueInJTable();
       //  tab1.setEnabled(false);
         tab1.setToolTipText("Select Any Row And  ( Choose Delete Button ) To Delete Room");
	...
	try
       					...
       					for(int i=0;i<ind.length;i++)
     					{
     						...
     						query="delete * from bedDetails where roomNo=";
     						query+=roomno[i];
     						//System.out.println(""+query);
     						inf.executeUpdateQuery(query);
     					}
