...
class staffSalary extends JFrame implements ActionListener,MouseListener,KeyListener
{
	...
	public staffSalary(JDesktopPane dp1)
	{
		...
		String head[]={"Sno.","Staff Code","Name","Salary","Advance","Due Salary","Paid Salary"};
			...
			if(getrwcount!=-1)
			{
				...
				if(tf_Pay.getText().equals("0.00"))
				{
					bt_PAID.setEnabled(false);
                                       bt_ADVANCE.setEnabled(false);
				}
				else
				{
					tf_Pay.requestFocus();
					bt_PAID.setEnabled(true);
                                        bt_ADVANCE.setEnabled(true);
				}
		...
		else if(ke.getSource()==tf_PayAdv && ke.getKeyCode()==ke.VK_ENTER)
		{
			...
			if(!tf_PayAdv.getText().trim().equals(""))
			{
				...
				//	System.out.println("getrwValue "+getrwValue);
					...
					//double showdiff=Double.valueOf(setpay)-Double.valueOf(setadvpay)+Double.valueOf(table.getValueAt(table.getSelectedRow(), 4).toString());
                                        double showdiff=Double.valueOf(setadvpay)+Double.valueOf(table.getValueAt(table.getSelectedRow(), 4).toString());
					System.out.println("showdiff "+showdiff);

                                        //double showdiff1=Double.valueOf(setpay)-Double.valueOf(setadvpay);
	...
	//	System.out.println("MON1 "+MON1);
		String dt1=MON1+"/"+"1"+"/"+YER1;
		
	//	System.out.println("dt1 "+dt1);
		String dt2=MON1+"/"+"28"+"/"+YER1;
	...
	//	System.out.println("dt2 "+dt2);
		...
		//	ZedInterface inf=(ZedInterface)Naming.lookup(getIPaddress.IP());
			...
			//	System.out.println(i+" "+loadSCode[i]);
				int c=0;
				String qry1="Select count(s_Code) from staff_AttendanceRegister where attend_Dt>=#"+dt1+"# And attend_Dt<=#"+dt2+"# And s_Code="+Integer.parseInt(loadSCode[i])+"";
		...
		//	String loadSal[][]=new String[getrwcnt][];
                        String tmp="Daily";
			qry="Select s_Code,s_Salary,s_SalaryBasis from staff_Details ";
                        System.out.println("qry "+qry);
			String loadSCandSalary[][]=inf.getResultSetAs_2D_Array(qry,3);
			if(loadSCandSalary!=null)
			{
				...
				//	System.out.println(j+""+loadSCandSalary[j][k]);
					String scode=loadSCandSalary[j][0];
					//System.out.println(j+""+loadSCandSalary[j][++k]);
					String salary=loadSCandSalary[j][1];
					double convrt=Double.valueOf(salary);
                                        System.out.println("convrt "+convrt+"    salary="+salary);
					for(int s=0;s<loadIDandATTD.length;s++)
					{
                                            
						if((Integer.parseInt(scode))==Integer.parseInt(loadIDandATTD[s][0]))
						{
                                                    String qury="select s_SalaryBasis from staff_Details where s_Code="+scode+"";
                                                    String item=inf.getSingleValue(qury);
							...
							//table.setValueAt(setsal,s,3);
                                                     ...
                                                     {
                                                         table.setValueAt((""+(new java.math.BigDecimal(convrt).setScale(2,java.math.BigDecimal.ROUND_HALF_UP))),s,3);
                                                     }
						}
			...
			if(loadSCode1!=null)
			{
				...
				//	System.out.println("lenof sal sum "+loadstafSAL);
							...
							//		System.out.println("loadPaidsal= "+loadPaidsal);
									...
									table.setValueAt((""+(new java.math.BigDecimal(loadPaidsal).setScale(2,java.math.BigDecimal.ROUND_HALF_UP))),i,6);
									/////////////////////////////////////////////
							}
							else
							{
								...
								table.setValueAt((""+(new java.math.BigDecimal(loadPaidsal).setScale(2,java.math.BigDecimal.ROUND_HALF_UP))),i,6);
									...
									String getpaidsalary=""+table.getValueAt(i,6);
								...
								//	System.out.println("SLADIFF "+SLADIFF);
									...
									if(loadADV!=null)
									{
										...
										table.setValueAt((""+(new java.math.BigDecimal(loadADV).setScale(2,java.math.BigDecimal.ROUND_HALF_UP))),i,4);
									}
									else
									{
										double loadAD=00.0;
										sum1.add(new java.math.BigDecimal(loadAD));
										table.setValueAt((""+(new java.math.BigDecimal(loadAD).setScale(2,java.math.BigDecimal.ROUND_HALF_UP))),i,4);
...
try
				...
				//	System.out.println("Mon "+Mon);
					...
					//	System.out.println("successful "+successful);
						...
						String gettotalad=""+table.getValueAt(getrw,4);
						...
						String getTOTALADVS=(""+(new java.math.BigDecimal(gettotalad).setScale(2,java.math.BigDecimal.ROUND_HALF_UP)));
					 	...
					 	{
					 		double showdiff=Double.valueOf(getTOTALADVS)-(Double.valueOf(tf_Pay.getText().trim())-Double.valueOf(getADVS));
					 		System.out.println("showdiff "+showdiff);
