...
public class Medical_Fitness_Certificate_Report implements ActionListener,KeyListener
{	
	...
	public Medical_Fitness_Certificate_Report(JDesktopPane desk)
	{	
		jtf=new JInternalFrame("Medical / Fitness Certificate Details",false,true,false,true);
		...
		String head[]={"S.No.","Issue Date","Name","Age","Staying At","Guardian","Suffering From","Surgery Of"};
		...
		column=jt.getColumn("Staying At");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(100);
		
		column=jt.getColumn("Guardian");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(120);

		column=jt.getColumn("Suffering From");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(100);
		...
		date1.setToolTipText("Select Date1");
		date2.setToolTipText("Select Date2");
	...
	private void showButton()
	{
		...
		{	
			...
			{	
				...
				String str1[]=inf.getResultSetAs_1D_Array("select date from medicalCertificate order by date");
			...
			sno=0;
			dtm.setRowCount(0);
			String pid[]=inf.getResultSetAs_1D_Array("select certificateNo from medicalCertificate where date>=#"+hdate1+"# and date<=#"+hdate2+"# order by date");
			if(pid!=null && pid.length!=0)
			{
				for(int l=0;l<pid.length;l++)
				{
					if(pid[l]!=null)
					{
						arr=inf.getResultSetAs_2D_Array("select date,name,age,stayingAt,guardian,sufferingFrom,surgeryOf from medicalCertificate where certificateNo="+Integer.parseInt(pid[l])+"",7);
						if(arr!=null)
						{
							hdate=arr[0][0];
							str=hdate.split("-");
