...
public class newSpectaclesDetail implements ActionListener,KeyListener
{	
	...
	public newSpectaclesDetail(JDesktopPane desk)
	{	
		...
		jl7=new JLabel("Select Category");
		...
		String head[]={"S.No.","Giving Date","ID","Name","Related With","Surgeon","Lens No","Total Amount","Due Amount","Pending"};
		...
		cn.add(jl4);
		jl7.setBounds(415,10,160,25);
		cn.add(jl7);
		
				
		cmb1.setBounds(575,10,130,25);
		cn.add(cmb1);
		cmb1.addItem("Select");
	...
	private void showButton()
	{
		try
		{
			...
			else
			{
					...
					sno=0;
					dtm.setRowCount(0);
					String cid[]=inf.getResultSetAs_1D_Array("select cID from customer_SpectsProvidation where date>=#"+hdate1+"# and date<=#"+hdate2+"#");
					for(int l=0;l<cid.length;l++)
					{
						String qry="select glassLensNo,givenDt,amt,dueAmt from customer_SpectsProvidation where cID='"+(cid[l])+"'";
						System.out.println("qry  "+qry);
						arr=inf.getResultSetAs_2D_Array("select glassLensNo,givenDt,amt,dueAmt from customer_SpectsProvidation where cID='"+(cid[l])+"'",4);
						if(cid[l]!=null && arr!=null)
						{
							str=arr[0][1].split("-");
							gyear=str[0];
							gmonth=str[1];
					...
					//		name=inf.getSingleValue("select drName from drDetail where drCode="+Integer.parseInt(name)+"");
							name="";
							String status=inf.getSingleValue("select status from customer_SpectsProvidation where cID='"+(cid[l])+"'");
							System.out.println("status="+status);
							arr1[0]=""+(++sno);						
							arr1[1]=gdate+"-"+gmonth+"-"+gyear;
							arr1[2]=cid[l];
