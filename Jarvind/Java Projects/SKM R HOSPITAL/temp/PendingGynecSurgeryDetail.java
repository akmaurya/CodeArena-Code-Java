...
public class PendingGynecSurgeryDetail implements ActionListener,KeyListener
{	
	...
	public PendingGynecSurgeryDetail(JDesktopPane desk)
	{	
		jtf=new JInternalFrame("Pending Gynec Surgery Details",false,true,false,true);
					...
					for(int l=0;l<pid.length;l++)
					{
						if(pid[l]!=null)
						{
							...
							if(Integer.parseInt(penmn)<=Integer.parseInt(opmonth))
							{
								...
								{
									String getID[][]=inf.getResultSetAs_2D_Array("Select distinct patientid,drcode from patient_Profile where patientid="+Integer.parseInt(pid[l])+"",2);
									if(getID!=null)
									{
										
										String drnm=inf.getSingleValue("Select drName from drDetail where drCode="+Integer.parseInt(getID[0][1])+"");
						...
						if(pid[l]!=null)
						{
							...
							if(Integer.parseInt(penmn)<=Integer.parseInt(opmonth))
							{
								if(Integer.parseInt(pendt)<=Integer.parseInt(opdate))
								{
									...
									if(getID!=null)
									{
										String name=inf.getSingleValue("Select drName from drDetail where drCode="+Integer.parseInt(code)+"");
