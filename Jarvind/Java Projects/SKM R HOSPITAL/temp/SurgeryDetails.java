...
public class SurgeryDetails implements ActionListener,KeyListener
{	
	...
	public SurgeryDetails(JDesktopPane desk)
	{	
		jtf=new JInternalFrame("Surgery Details",false,true,false,true);
		...
		String head[]={"S.No.","OPDate","PID","Name","Related With","Surgery Of","Surgery","Surgeon Name","Charge"};
		...
		column=jt.getColumn("Surgery Of");
      		column.setCellRenderer(CRLeft);
		...
		column=jt.getColumn("Surgeon Name");
      		column.setCellRenderer(CRLeft);
       		column.setPreferredWidth(150);
		...
		{	
			...
			if(name.equals("All"))
			{
				for(int l=0;l<pid.length;l++)
				{
					if(pid[l]!=null)
					{
						arr=inf.getResultSetAs_2D_Array("select Surgeon_name,Surgery_SISSC,Surgery_PHACO,Surgery_TRAB,Surgery_DCR,Surgery_SQUINT,Surgery_ENT,Surgery_PTR,Surgery_SecIOL,amount from eye_Surgery_Detail where Patient_Id="+Integer.parseInt(pid[l])+"",10);
						
						String pname=inf.getSingleValue("select name from patient_Profile where patientid="+Integer.parseInt(pid[l])+"");
						String right=inf.getSingleValue("select Selected_eye_RE from eye_Surgery_Detail where Patient_Id="+Integer.parseInt(pid[l])+"");
						if(Integer.parseInt(right)==1)
						{
							right="Right";
							arr1[5]="Right Eye";
						}
						String left=inf.getSingleValue("select Selected_eye_LE from eye_Surgery_Detail where Patient_Id="+Integer.parseInt(pid[l])+"");
			...
			else
			{
				...
				for(int l=0;l<pid.length;l++)
				{	
					arr=inf.getResultSetAs_2D_Array("select Surgeon_name,Surgery_SISSC,Surgery_PHACO,Surgery_TRAB,Surgery_DCR,Surgery_SQUINT,Surgery_ENT,Surgery_PTR,Surgery_SecIOL,amount from eye_Surgery_Detail where Patient_Id="+Integer.parseInt(pid[l])+" and Surgeon_name='"+name+"'",10);
