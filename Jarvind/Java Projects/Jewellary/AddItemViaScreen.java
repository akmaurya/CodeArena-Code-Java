...
public class AddItemViaScreen extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener
{
	Container con;
	JDialog frm;
	...
	public AddItemViaScreen(JDesktopPane desk)
	{
		frm=new JDialog(new Frame(),"Add New Item",true);
		...
		editBtn.setBounds(42,240,100,25);	
					...
					if(openTf.getText().trim().length()>0)
					{
						if(tounchTf.getText().trim().length()>0)
						{
							if(pieceTf.getText().trim().length()>0)
							{
		    	    		    ...
		    	    		    System.out.println("update itemList set categoryName='"+categoryCb.getSelectedItem()+"',itemName='"+itemTf.getText().trim().toUpperCase()+"',weight="+openTf.getText()+",piece="+pieceTf.getText()+" where categoryName='"+str1+"' and itemName='"+str2+"' and weight="+sdf+" and piece="+p2+" ");
		    	    		    st.executeUpdate("update itemList set categoryName='"+categoryCb.getSelectedItem()+"',itemName='"+itemTf.getText().trim().toUpperCase()+"',weight="+openTf.getText()+", piece="+pieceTf.getText()+" where categoryName='"+str1+"' and itemName='"+str2+"' and weight="+sdf+" and piece="+p2+" ");
						...
						if(tounchTf.getText().trim().length()>0)
						{
							if(pieceTf.getText().trim().length()>0)
							{
	        		    		...
	        		    		else			
    	        				{
			            			...
			            			//reset();
			            			//if(Sale.categoryCb.getSelectedItem().toString().equals(categoryCb.getSelectedItem().toString()))
			            			//{
			            				Sale.itemCb.removeAllItems();
        		    					rs=st.executeQuery("select * from itemList  order by itemName");
        		    					while(rs.next())
        		    					{
				        		    		Sale.itemCb.addItem(rs.getString("itemName"));        		    	
        		    					}
        		    					Sale.itemCb.setSelectedItem(itemTf.getText().trim().toUpperCase());
		           					//}
		           					//if(Sale.categoryCb1.getSelectedItem().toString().equals(categoryCb.getSelectedItem().toString()))
			            			//{
			            			//	Sale.itemCb1.removeAllItems();
        		    					rs=st.executeQuery("select * from itemList order by itemName");
        		    					while(rs.next())
        		    					{
				        		    		Sale.itemCb1.addItem(rs.getString("itemName"));        		    	
        		    					}
        		    					Sale.itemCb1.setSelectedItem(itemTf.getText().trim().toUpperCase());
		           					//}
		           					frm.dispose();
		           				}	
            					cn.close();
	...
	/*public void mouseClicked(MouseEvent e)
	{
			...
			try
			{
	            	...
	            	Statement st=cn.createStatement();
        		    ResultSet rs;
        		    rs=st.executeQuery("select * from itemList where categoryName='"+categoryCb.getSelectedItem()+"' and  itemName='"+itemTf.getText()+"'");
        		    if(rs.next())
        		    {
        		    	openTf.setText(rs.getString("openingWt"));        		    	
        		    }
		           	cn.close();
            }	
			catch(Exception x)
