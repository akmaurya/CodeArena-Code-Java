...
class patchForPurchase1 extends JFrame
{
	public static void main(String args[])
	{
		patchForPurchase1 pt=new patchForPurchase1();
		pt.setSize(200,300);
		pt.setVisible(true);
		try
		{
        	...
        	Connection cn=DriverManager.getConnection("jdbc:odbc:jweldsn","zed","zed"); 
           	...
           	ResultSet rs=st.executeQuery("select all distinct p_name from purchase");
			int c=0,i=0;
			while(rs.next())
			{
				c++;
			...
			rs=st.executeQuery("select all distinct p_name from purchase");
			while(rs.next())
			{
				partyList[i]=rs.getString(1);
				i++;
			}
			System.out.println(partyList[4]);
			//for(i=0;i<partyList.length;i++)
			{
				//System.out.println("party goes to update...."+partyList[i]+"-"+i);
				pt.updatedata(partyList[4]);
			}
		...
		{
			System.out.println("Error...90..."+ex);
		}
