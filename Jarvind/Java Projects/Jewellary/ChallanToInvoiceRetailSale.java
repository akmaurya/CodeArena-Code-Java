...
public class ChallanToInvoiceRetailSale extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener,InternalFrameListener,TableModelListener
{
	Container con;
	JInternalFrame frm;
	JButton addBtn,deleteBtn,exitBtn,resetBtn;
	static JComboBox categoryCb,itemCb,categoryCb1,itemCb1;
	static JTextField partyCb;
	JComboBox gmOrBhari;
	...
	double curBalance=0;
	JDesktopPane desk;
	JLabel drCrLb,drCrLb1,lastTounchLb,lastTounchLb1;
	...
	JTextField discounttf,billNoLb;
	JLabel totalPaybleAmtL,vatAmtL,netPaybleAmtL;
	JButton dateBtn,editBtn;	
	JPopupMenu pm,pm1;
	JMenuItem deletePM,deletePM1;
		
	public ChallanToInvoiceRetailSale(JDesktopPane desk,int billno)
	{
		System.out.println("called...");
		this.desk=desk;
		frm=new JInternalFrame("Convert Challan To Invoice # Retail",false,true,false,true);
		...
		addBtn=new JButton("Convert Challan To Invoice");
		deleteBtn=new JButton("Cancel Challan");
		editBtn=new JButton("Update");
		exitBtn=new JButton("Close");
		resetBtn=new JButton("Reset");
		String heading[]={"Item Category","Item Name","Gross Wt.","Stone Wt.","Stone Rate","Net Wt.","Rate","Making Charge","Amount"};
		dtm=new MyDefaultTableModel(heading,0);//DefaultTableModel(heading,0);
		grid=new JTable(dtm);
		JScrollPane jsp=new JScrollPane(grid);
		String blankrow[]=new String[9];
		dtm.addRow(blankrow);
		...
		String heading1[]={"Item Category","Item Name","Gross Wt.","Stone Wt.","Stone Rate","Net Wt.","Rate","Making Charge","Amount"};
		dtm1=new MyDefaultTableModel(heading1,0);
		...
		totalPurWtL.setBounds(630,240,130,20);
		totalAmountL.setBounds(980,240,130,20);
		...
		totalPurWtL1.setBounds(630,470,130,20);
		totalAmountL1.setBounds(980,470,130,20);
		...
		addBtn.setBounds(10,590,183,25);
		deleteBtn.setBounds(195,590,183,25);
		editBtn.setBounds(380,590,183,25);
		printBtn.setBounds(565,590,183,25);
		resetBtn.setBounds(750,590,183,25);
		exitBtn.setBounds(935,590,183,25);
		...
		billNoLb.setHorizontalAlignment(JTextField.CENTER);
	...
	public void keyPressed(final KeyEvent e)
	{
		...
		SwingUtilities.invokeLater(new Runnable()
		{
			public void run()
			{
				...
				{
						if ( (nRow > -1 && nRow < m_table.getRowCount()) && (nCol > -1 && nCol < 8)  && (nCol !=  7))	//(m_table.getColumnCount()-1)))
						{
							...
							else if(nCol==6)	//pressed ENTER on RATE
							{
						...
						else if( (nRow == (m_table.getRowCount()-1) && (nCol == 7)) )	//(m_table.getColumnCount()-1))) )
						{
							newRowFlag=false;
							String blankrow[]=new String[9];
							if(e.getSource()==grid)
...
public void tableChanged(TableModelEvent e)
{
	...
	if(newRowFlag==true)
	{
		...
		else if(nCol==1)
		{
			...
			String lasttounch=getLastTounch(partyCb.getText().toString(),objSel.toString());
			table.setValueAt(lasttounch,nRow,6);
		}
		if((nCol==2 || nCol==3  || nCol==4 || nCol==6 || nCol==7) && errorInTableFlag==false)
		{
			String msg="";
			...
			else if(nCol==6)	msg="Error! Invalid character found in RATE.";
			else if(nCol==7)	msg="Error! Invalid character found in MAKING CHARGE.";
			...
			try
			{
						...
						if(nCol==2 || nCol==3)
						{
							...
							if(table.getValueAt(nRow, 0).toString().trim().equalsIgnoreCase("gold")==true || table.getValueAt(nRow, 0).toString().trim().equalsIgnoreCase("silver")==true)
							{
								...
								table.setValueAt(""+(new BigDecimal(netweight)).setScale(3,5),nRow,5);
							}
							else	//if stone
							{
							...
							if(table==grid)
							{
								totalPurWtL.setText(""+(new BigDecimal(getTotalInDecimal(table,5))).setScale(3,5));
							}
							else
							{
								totalPurWtL1.setText(""+(new BigDecimal(getTotalInDecimal(table,5))).setScale(3,5));
							}
						...
						if(nCol==4)
						{
							...
							table.setValueAt(""+(new BigDecimal(amount)).setScale(2,5), nRow, 8);

							if(table==grid)
							{
								totalAmountL.setText(""+(new BigDecimal(getTotalInDecimal(table,8))).setScale(2,5));
								calculateNetPaybleAmt();
							}
							else
							{
								totalAmountL1.setText(""+(new BigDecimal(getTotalInDecimal(table,8))).setScale(2,5));
								calculateNetPaybleAmt();
							}	
						}
						/////////////////////////////////////////////////////
						...
						*/
						...
						if(nCol==7)
						{
							String temprate=(""+table.getValueAt(nRow, 7)).trim();
							if(temprate.equalsIgnoreCase("")==true || temprate.equalsIgnoreCase("null")==true)
								temprate="0";
							double amount=0;
							...
							if(table.getValueAt(nRow, 0).toString().equalsIgnoreCase("")==false)
								amount=amount+Double.valueOf((""+table.getValueAt(nRow, 8)).trim());
...
class ColorColumnRenderer extends DefaultTableCellRenderer 
{
	     ...
	     boolean hasFocus, int row, int column) 
   {
	...
	private void calculateFineWeight(JTable m_table,int nRow)
	{
							...
							String tempfineweight=""+(new BigDecimal(fineweight)).setScale(3,5);
							...
							if(m_table==grid)
							{
								totalPurFineWtL.setText(""+(new BigDecimal(getTotalInDecimal(m_table,11))).setScale(3,5));	//total fine weight of gold
								totalSilverFineWtL.setText(""+(new BigDecimal(getTotalInDecimal(m_table,12))).setScale(3,5));
							}
							else
							{
								totalPurFineWtL1.setText(""+(new BigDecimal(getTotalInDecimal(m_table,11))).setScale(3,5));	//total fine weight of gold
								totalSilverFineWtL1.setText(""+(new BigDecimal(getTotalInDecimal(m_table,12))).setScale(3,5));
							}
							//System.out.println("ok reached ------->>>>>>>>>>>>>>>>>>>>>>");
	...
	private void calculateAmount(JTable table,int nRow)
	{
							...
							String temprate=(""+table.getValueAt(nRow, 6));
							if(temprate.equalsIgnoreCase("")==true || temprate.equalsIgnoreCase("null")==true)
								temprate="0";
							double amount=0;
							if(table.getValueAt(nRow, 0).toString().equalsIgnoreCase("gold")==true)
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if(e.getSource()==partyCb)
		{
			try
			{
				String str=partyCb.getText().toString();
	...
	public void keyPressed(KeyEvent e)
	{
		...
		if(e.getSource()==billNoLb && e.getKeyCode()==10)
		{
			showBill();	
		}
		else if(e.getSource()==dateBtn && e.getKeyCode()==10)
		{
			Cal cl=new Cal();
	        dateTf.setText(cl.getdate());
		}
	...
	*/
	...
	private boolean rollBackData()
	{
		try
		{
			//Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
	   		...
	   		rs=st.executeQuery("select * from saleRetailchallan where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='s'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("itemcode");
	   			String v6=rs.getString("grosswt");
	   			rsTemp=stTemp.executeQuery("select * from itemList where itemcode="+v2);
				if(rsTemp.next())
				{
					BigDecimal bd1=rsTemp.getBigDecimal("weight").setScale(3,5);
					bd1=(bd1.add(new BigDecimal(v6))).setScale(3,5);
					stTemp.executeUpdate("update itemList set weight="+bd1+" where itemcode="+v2+"");
				}
			}
			
	   		rs=st.executeQuery("select * from saleRetailchallan where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='a'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("itemcode");
	   			String v6=rs.getString("grosswt");
				...
				{
					BigDecimal bd1=rsTemp.getBigDecimal("weight").setScale(3,5);
					bd1=(bd1.subtract(new BigDecimal(v6))).setScale(3,5);
					stTemp.executeUpdate("update itemList set weight="+bd1+" where itemcode="+v2+"");
				}
			}
			return true;
		...
		{
			System.out.println("error 199 ..."+x);
		}
		return false;
	}
	
	private boolean cancelledChallan()
	{
		int response=JOptionPane.showConfirmDialog(null,"Are you sure about cancellation of this CHALLAN?","Confirmation",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
		...
		try
		{
	   		Connection cn=Utility.makeConnection();
		  	Statement st=cn.createStatement();
	    	int psno=Integer.parseInt(billNoLb.getText());
			st.executeUpdate("update saleRetailChallan set challanstatus='c' where s_s_no="+psno+"");		
			cn.close();
		}			
		catch(Exception ex)
		{
			System.out.println("Error no. 829..."+ex);
		}
	...
	private boolean saveData()
	{
		...
		int response=JOptionPane.showConfirmDialog(null,"Are you sure about this CHALLAN to INVOICE conversion?","Confirmation",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
		if(response==JOptionPane.NO_OPTION)
			return false;
		...
		try
		{
			if(dateTf.getText().trim().length()>0)
			{
				//if(grid.getRowCount()>0 || grid1.getRowCount()>0)
				{
		    	    		    ...
		    	    		    String partynm=""+partyCb.getText();
		    	    	    ...
		    	    	    {
								String v1=""+grid.getValueAt(i,8);//amount
								if(v1.equals("null")==false && v1.equals("")==false)
								{
									...
									qry="insert into saleRetailInvoice values("+psno+",'"+partynm+"','"+dt+"',"+itemcode;
									for(int col=2;col<=8;col++)
									{
										if((""+grid.getValueAt(i,col)).equals("null")==true || (""+grid.getValueAt(i,col)).equals("")==true)
											qry=qry+",0";
										else	
		    	        	...
		    	        	{
								String v1=""+grid1.getValueAt(i,8);//amount
								if(v1.equals("null")==false && v1.equals("")==false)
								{
									...
									for(int col=2;col<=8;col++)
									{
										if((""+grid1.getValueAt(i,col)).equals("null")==true || (""+grid1.getValueAt(i,col)).equals("")==true)
											qry=qry+",0";
										else	
							...
							//String qry1="insert into saleInvoiceAmtDetails values("+psno+","+Double.valueOf(totalPaybleAmtL.getText())+","+Double.valueOf(vatAmtL.getText())+","+Double.valueOf(disamtvar)+","+Double.valueOf(netPaybleAmtL.getText())+","+Double.valueOf(depositeTf.getText())+")";
							String qry1="insert into saleRetailInvoiceAmtDetails values("+psno+","+Double.valueOf(totalPurWtL.getText())+","+Double.valueOf(totalAmountL.getText())+","+Double.valueOf(totalPurWtL1.getText())+","+Double.valueOf(totalAmountL1.getText())+","+Double.valueOf(totalPaybleAmtL.getText())+","+Double.valueOf(vatAmtL.getText())+","+Double.valueOf(disamtvar)+","+Double.valueOf(netPaybleAmtL.getText())+","+Double.valueOf(depositeTf.getText())+")";
							...
							stTemp.executeUpdate(qry1);
									...
									///////////////////////////////////////////	
	            					rsTemp=stTemp.executeQuery("select * from partyDetails where partyName='"+partynm+"' and p_type='s'");
									if(rsTemp.next())
									{
										BigDecimal bd1=rsTemp.getBigDecimal("currentWt");	//wt. of gold
										BigDecimal bd2=rsTemp.getBigDecimal("currentSilverWt");	//wt. of silver
										BigDecimal bd3=rsTemp.getBigDecimal("currentAmt");	//current amount
										
										bd1=bd1.add(new BigDecimal(Double.valueOf(totalPurFineWtL.getText())-Double.valueOf(totalPurFineWtL1.getText())));
										bd2=bd2.add(new BigDecimal(Double.valueOf(totalSilverFineWtL.getText())-Double.valueOf(totalSilverFineWtL1.getText())));
										bd3=bd3.add(new BigDecimal(Double.valueOf(netPaybleAmtL.getText())-Double.valueOf(depositeTf.getText())));

										bd1=bd1.setScale(3,5);
	            						bd2=bd2.setScale(3,5);
	            						bd3=bd3.setScale(2,5);
										stTemp.executeUpdate("update partyDetails set currentWt="+bd1+",currentSilverWt="+bd2+",currentAmt="+bd3+" where partyName='"+partynm+"' and p_type='s'");
									}
									*/
									//////////////////////////////
									//REQUIRED CODE OF PAYMENT
									//////////////////////////////
	...
	private boolean rollBackInvoiceData()
	{
		try
		{
			//Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
	   		...
	   		ResultSet rs,rsTemp;
	   		System.out.println("ok1 ...");
	   		rs=st.executeQuery("select * from saleRetailinvoice where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='s'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("itemcode");
	   			String v6=rs.getString("grosswt");
	   		...
	   		rs=st.executeQuery("select * from saleRetailinvoice where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='a'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("iTEMcode");
	   			String v6=rs.getString("grosswt");
			...
			///////////////////////////////////////////	
			String partynm="";
			rs=st.executeQuery("select p_name from saleinvoice where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			if(rs.next())
			{
				partynm=rs.getString(1);
			}
	   		System.out.println("ok1111 ...");
			
			double goldwt=0,goldwt1=0,silverwt=0,silverwt1=0,netamt=0,depositeamt=0;
			rs=st.executeQuery("select sale_goldfinewt,sale_silverfinewt,adj_goldfinewt,adj_silverfinewt,netpaybleamt,depositeamt from saleinvoiceAmtDetails where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			if(rs.next())
			{
				goldwt=rs.getDouble(1);
				silverwt=rs.getDouble(2);
				goldwt1=rs.getDouble(3);
				silverwt1=rs.getDouble(4);
				netamt=rs.getDouble(5);
				depositeamt=rs.getDouble(6);
			...
			if(rsTemp.next())
			{
				...
				BigDecimal bd2=rsTemp.getBigDecimal("currentSilverWt");	//wt. of silver
				BigDecimal bd3=rsTemp.getBigDecimal("currentAmt");	//current amount
			System.out.println("ok0000 ...>"+bd1+","+bd2+","+bd3);
			System.out.println("ok0000 ...>"+goldwt+","+goldwt1);
			System.out.println("ok0000 ...>"+silverwt+","+silverwt1);
			System.out.println("ok0000 ...>"+netamt+","+depositeamt);
			
				bd1=bd1.subtract(new BigDecimal(goldwt-goldwt1));
				bd2=bd2.subtract(new BigDecimal(silverwt-silverwt1));
				bd3=bd3.subtract(new BigDecimal(netamt-depositeamt));
			...
			*/
			...
			st.executeUpdate("delete * from saleInvoice where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			st.executeUpdate("delete * from lasttounch where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			st.executeUpdate("delete * from saleInvoiceAmtDetails where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
	...
	private boolean updateData()
	{
		...
		int response=JOptionPane.showConfirmDialog(null,"Are you sure about updation of existing INVOICE?","Confirmation",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
		if(response==JOptionPane.NO_OPTION)
			return false;
		...
		try
		{
			if(dateTf.getText().trim().length()>0)
			{
				//if(grid.getRowCount()>0 || grid1.getRowCount()>0)
				{
							...
							if(disamtvar.equals(""))	disamtvar="0";		
							//String qry1="insert into saleInvoiceAmtDetails values("+psno+","+Double.valueOf(totalPaybleAmtL.getText())+","+Double.valueOf(vatAmtL.getText())+","+Double.valueOf(disamtvar)+","+Double.valueOf(netPaybleAmtL.getText())+","+Double.valueOf(depositeTf.getText())+")";
							String qry1="insert into saleRetailChallanAmtDetails values("+psno+","+Double.valueOf(totalPurWtL.getText())+","+Double.valueOf(totalAmountL.getText())+","+Double.valueOf(totalPurWtL1.getText())+","+Double.valueOf(totalAmountL1.getText())+","+Double.valueOf(totalPaybleAmtL.getText())+","+Double.valueOf(vatAmtL.getText())+","+Double.valueOf(disamtvar)+","+Double.valueOf(netPaybleAmtL.getText())+","+Double.valueOf(depositeTf.getText())+")";
							System.out.println(qry1);
							stTemp.executeUpdate(qry1);
	...
	*/
	...
	private boolean showBill()
	{
		try
		{
					String tableNameSaleXX="saleRetailchallan",tableNameSaleDetailsXX="saleRetailChallanAmtDetails";
					String billno=billNoLb.getText();
					try
					{
						Integer.parseInt(billno);
					}
					catch(Exception ex)
					{
						JOptionPane.showMessageDialog(null,"Error! Invalid CHALLAN NO. found.");
						billNoLb.selectAll();
						return false;
					}
					//Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
        		    ...
        		    Statement st1=cn.createStatement();
        		    ResultSet rs,rs1;
        		    int challanno=Integer.parseInt(billno);
					String partynm=""+partyCb.getText();

        		    	reset();
					int btnflag=0;
								rs=st.executeQuery("select * from saleRetailinvoice where s_s_no="+challanno+"");
								if(rs.next())
								{
									int response=JOptionPane.showConfirmDialog(null,"This challan no. is already converted to invoice.\nDo you want to open existing invoice for any editing?","Confirmation",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
									if(response==JOptionPane.NO_OPTION)
									{
											reset();
											return false;
									}
									tableNameSaleXX="saleRetailinvoice";
									tableNameSaleDetailsXX="saleRetailInvoiceAmtDetails";
									btnflag=1;
								}

        		    rs=st.executeQuery("select * from "+tableNameSaleXX+" where s_s_no="+challanno+"");
        		    if(rs.next())
        		    {
        		    	
        		    	partyCb.setText(rs.getString("p_name"));
        		    	billNoLb.setText(billno);
						if(btnflag==1)
						{
        		    	...
        		    	rs=st.executeQuery("select * from "+tableNameSaleXX+" where s_s_no="+challanno+" and t_type='s'");
        		    	while(rs.next())
        		    	{
        		    		String arr[]=new String[9];
        		    		int itemcode=rs.getInt("itemcode");
	        		    	rs1=st1.executeQuery("select * from itemlist where itemcode="+itemcode+"");
        		    		if(rs1.next())
        		    		{
        		    			arr[0]=rs1.getString("categoryname");
        		    			arr[1]=rs1.getString("itemname");        		    			
        		    		}	
        		    		arr[2]=rs.getString(5);
        		    		arr[3]=rs.getString(6);
        		    		arr[4]=rs.getString(7);
        		    		arr[5]=rs.getString(8);
        		    		arr[6]=rs.getString(9);
        		    		arr[7]=rs.getString(10);
        		    		arr[8]=rs.getString(11);
        		    		
        		    		dtm.addRow(arr);
        		    	}
						
        		    	rs=st.executeQuery("select * from "+tableNameSaleXX+" where s_s_no="+challanno+" and t_type='a'");
        		    	while(rs.next())
        		    	{
        		    		String arr[]=new String[9];
        		    		int itemcode=rs.getInt("itemcode");
        		    	...
        		    	rs=st.executeQuery("select * from "+tableNameSaleDetailsXX+" where s_s_no="+challanno+"");
						if(rs.next())
						{
							totalPurWtL.setText(rs.getString("sale_netwt"));
							totalAmountL.setText(rs.getString("sale_amt"));
							//totalPurFineWtL.setText(rs.getString("sale_goldfinewt"));
							//totalSilverFineWtL.setText(rs.getString("sale_silverfinewt"));

							totalPurWtL1.setText(rs.getString("adj_netwt"));
							totalAmountL1.setText(rs.getString("adj_amt"));
							//totalPurFineWtL1.setText(rs.getString("adj_goldfinewt"));
							//totalSilverFineWtL1.setText(rs.getString("adj_silverfinewt"));
																
							totalPaybleAmtL.setText(rs.getString("paybleamt"));
							vatAmtL.setText(rs.getString("vatamt"));
							discounttf.setText(rs.getString("discount"));
							netPaybleAmtL.setText(rs.getString("netpaybleamt"));
							depositeTf.setText(rs.getString("depositeamt"));
						}
	...
	class printbill extends JFrame
    {
                public printbill()
                {
                        try
                        {
			        		    ...
			        		    String firmname=""+partyCb.getText();;
        					    String billno="";
		        ...
		        try
        		{
					...
					System.out.println("select slx.field3,sl.s_date from Sale_x_temp slx,Sale_temp sl where slx.s_s_no="+Integer.parseInt(billno)+" and sl.s_s_no=slx.s_s_no");
   	    		    ...
   	    		    if(rs.next())
   	    		    {
		    	    	...
		    	    	rs=st.executeQuery("select * from salechallan where s_s_no<"+Integer.parseInt(billno)+" and p_name='"+partyCb.getText()+"' order by s_date desc");
		    	    	if(rs.next())
		    	    	{
		    	    		g2.setFont(fp9);
		    		    	fm=g2.getFontMetrics();
