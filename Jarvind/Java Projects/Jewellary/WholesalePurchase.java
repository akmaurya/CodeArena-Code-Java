...
public class WholesalePurchase extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener,InternalFrameListener,TableModelListener
{
	Container con;
	JInternalFrame frm;
	...
	public WholesalePurchase(JDesktopPane desk,int billno)
	{
		this.desk=desk;
		frm=new JInternalFrame("Purchase Items",false,true,false,true);
		...
		JLabel lab23=new JLabel("Purcahse No.",JLabel.RIGHT);
		...
		addBtn=new JButton("Save Purchasing");
		...
		totalPurWtL=new JLabel("0",JLabel.LEFT);
		totalPurFineWtL=new JLabel("0",JLabel.LEFT);
		...
		lastBal3L=new JLabel("0",JLabel.RIGHT);//RIGHT
		...
		editBtn.setBounds(287,590,275,25);
		resetBtn.setBounds(564,590,275,25);
		exitBtn.setBounds(841,590,275,25);
		...
		*/
		...
		JLabel l1=new JLabel("PURCHASE DETAILS :::");
...
*/	
	private boolean cancelledChallan()
	{
		...
		try
		{
			...
			st.executeUpdate("update saleChallan set challanstatus='c' where s_s_no="+psno+"");		
			cn.close();
		}			
		catch(Exception ex)
		{
	...
	private boolean saveData()
	{
		...
		int response=JOptionPane.showConfirmDialog(null,"Are you sure?","Confirmation",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
		if(response==JOptionPane.NO_OPTION)
			return false;
				...
				//if(grid.getRowCount()>0 || grid1.getRowCount()>0)
				{
		    	   		    ...
		    	   		    for(int i=0;i<grid.getRowCount();i++)
		    	    	    {
								...
								if(v1.equals("null")==false && v1.equals("")==false)
								{
									...
									qry="insert into purchase values("+psno+",'"+partynm+"','"+dt+"',"+itemcode;
									...
									//Update LAST TOUNCH
									//////////////////////
									if((""+grid.getValueAt(i,6)).equals("null")==true)
										st.executeUpdate("insert into LastTounch values("+psno+",'"+partynm+"',0,"+itemcode+",'p')");
									else	
									{
										Double tnch=Double.valueOf(""+grid.getValueAt(i,6));
										st.executeUpdate("insert into LastTounch values("+psno+",'"+partynm+"',"+tnch+","+itemcode+",'p')");
									}
		    	    		...
		    	    		for(int i=0;i<grid1.getRowCount();i++)
		    	        	{
								...
								if(v1.equals("null")==false && v1.equals("")==false)
								{
									...
									//////////////////////
									if((""+grid.getValueAt(i,6)).equals("null")==true)
										st.executeUpdate("insert into LastTounch values("+psno1+",'"+partynm+"',0,"+itemcode+",'a')");
									else	
									{
										Double tnch=Double.valueOf(""+grid.getValueAt(i,6));
										st.executeUpdate("insert into LastTounch values("+psno1+",'"+partynm+"',"+tnch+","+itemcode+",'a')");
									}
							...
							String qry1="insert into purchaseAmtDetails values("+psno+","+Double.valueOf(totalPurWtL.getText())+","+Double.valueOf(totalAmountL.getText())+","+Double.valueOf(totalPurFineWtL.getText())+","+Double.valueOf(totalSilverFineWtL.getText())+","+Double.valueOf(totalPurWtL1.getText())+","+Double.valueOf(totalAmountL1.getText())+","+Double.valueOf(totalPurFineWtL1.getText())+","+Double.valueOf(totalSilverFineWtL1.getText())+","+Double.valueOf(totalPaybleAmtL.getText())+","+Double.valueOf(vatAmtL.getText())+","+Double.valueOf(disamtvar)+","+Double.valueOf(netPaybleAmtL.getText())+","+Double.valueOf(depositeTf.getText())+")";
							...
							stTemp.executeUpdate(qry1);
									...
									///////////////////////////////////////////	
	            					rsTemp=stTemp.executeQuery("select * from partyDetails where partyName='"+partynm+"' and p_type='p'");
									if(rsTemp.next())
									{
										...
										bd1=bd1.subtract(new BigDecimal(Double.valueOf(totalPurFineWtL.getText())-Double.valueOf(totalPurFineWtL1.getText())));
										bd2=bd2.subtract(new BigDecimal(Double.valueOf(totalSilverFineWtL.getText())-Double.valueOf(totalSilverFineWtL1.getText())));
										bd3=bd3.subtract(new BigDecimal(Double.valueOf(netPaybleAmtL.getText())-Double.valueOf(depositeTf.getText())));

										bd1=bd1.setScale(3,5);
	            						bd2=bd2.setScale(3,5);
	            						bd3=bd3.setScale(2,5);
										stTemp.executeUpdate("update partyDetails set currentWt="+bd1+",currentSilverWt="+bd2+",currentAmt="+bd3+" where partyName='"+partynm+"' and p_type='p'");
									}
	...
	private boolean rollBackInvoiceData()
	{
		try
		{
			//Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
	   		...
	   		ResultSet rs,rsTemp;
	   		System.out.println("ok1 ...");
	   		rs=st.executeQuery("select * from purchase where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='p'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("itemcode");
	   			String v6=rs.getString("grosswt");
	   		...
	   		rs=st.executeQuery("select * from purchase where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='a'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("icode");
	   			String v6=rs.getString("grosswt");
	   			rsTemp=stTemp.executeQuery("select * from itemList where itemcode="+v2);
				if(rsTemp.next())
				{
			...
			///////////////////////////////////////////	
			String partynm="";
			rs=st.executeQuery("select p_name from purchase where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			if(rs.next())
			{
			...
			rs=st.executeQuery("select sale_goldfinewt,sale_silverfinewt,adj_goldfinewt,adj_silverfinewt,netpaybleamt,depositeamt from purchaseAmtDetails where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			if(rs.next())
			{
			...
			System.out.println("ok0000 ...>"+netamt+","+depositeamt);
				...
				bd1=bd1.add(new BigDecimal(goldwt-goldwt1));
				bd2=bd2.add(new BigDecimal(silverwt-silverwt1));
				bd3=bd3.add(new BigDecimal(netamt-depositeamt));
			...
			st.executeUpdate("delete * from purchase where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			st.executeUpdate("delete * from lasttounch where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
			st.executeUpdate("delete * from purchaseAmtDetails where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
	...
	private boolean updateData()
	{
		...
		int response=JOptionPane.showConfirmDialog(null,"Are you sure about updation of existing Purchase Information?","Confirmation",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
		if(response==JOptionPane.NO_OPTION)
			return false;
	...
	private int getCurrentBillNo()
	{
			try
			{
	            	...
	            	Statement st=cn.createStatement();
        		    ResultSet rs;
		    	    		    rs=st.executeQuery("select max(s_s_no) from purchase");
		    	    		    if(rs.next())
		    	    		    {
		    	    		    	psno=rs.getInt(1)+1;
		    	    		    }
	...
	*/
	...
	private boolean showBill()
	{
		try
		{
					String tableNameSaleXX="purchase",tableNameSaleDetailsXX="purchaseAmtDetails";
					...
					int btnflag=0;
								...
								if(rs.next())
								{
									int response=JOptionPane.showConfirmDialog(null,"Do you want to open existing Purchase Bill for any editing?","Confirmation",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
									if(response==JOptionPane.NO_OPTION)
									{
											reset();
											return false;
									}
									tableNameSaleXX="purchase";
									tableNameSaleDetailsXX="purchaseAmtDetails";
									btnflag=1;
								}
        		    ...
        		    if(rs.next())
        		    {
        		    	...
        		    	rs=st.executeQuery("select * from "+tableNameSaleXX+" where s_s_no="+challanno+" and t_type='p'");
        		    	while(rs.next())
        		    	{
        		    		...
        		    		arr[9]=rs.getString(12);
        		    		arr[10]=rs.getString(13);
        		    		arr[11]=rs.getString(14);
        		    		arr[12]=rs.getString(15);
        		    		dtm.addRow(arr);
        		    	}
