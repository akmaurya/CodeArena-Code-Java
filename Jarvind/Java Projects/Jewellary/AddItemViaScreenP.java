...
public class AddItemViaScreenP extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener
{
	Container con;
	JDialog frm;
	...
	public AddItemViaScreenP(JDesktopPane desk)
	{
						...
						if(tounchTf.getText().trim().length()>0)
						{
							if(pieceTf.getText().trim().length()>0)
							{
	        		    		...
	        		    		else			
    	        				{
			            			...
			            			//reset();
			            			if(Purchase.categoryCb.getSelectedItem().toString().equals(categoryCb.getSelectedItem().toString()))
			            			{
			            				Purchase.itemCb.removeAllItems();
        		    					rs=st.executeQuery("select * from itemList where categoryName='"+categoryCb.getSelectedItem()+"' order by itemName");
        		    					while(rs.next())
        		    					{
				        		    		Purchase.itemCb.addItem(rs.getString("itemName"));        		    	
        		    					}
        		    					Purchase.itemCb.setSelectedItem(itemTf.getText().trim().toUpperCase());
		           					}
		           					if(Purchase.categoryCb1.getSelectedItem().toString().equals(categoryCb.getSelectedItem().toString()))
			            			{
			            				Purchase.itemCb1.removeAllItems();
        		    					rs=st.executeQuery("select * from itemList where categoryName='"+categoryCb.getSelectedItem()+"' order by itemName");
        		    					while(rs.next())
        		    					{
				        		    		Purchase.itemCb1.addItem(rs.getString("itemName"));        		    	
        		    					}
        		    					Purchase.itemCb1.setSelectedItem(itemTf.getText().trim().toUpperCase());
		           					}
		           					frm.dispose();
		           				}	
            					cn.close();
