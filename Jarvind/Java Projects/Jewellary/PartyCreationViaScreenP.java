...
public class PartyCreationViaScreenP extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener
{
	Container con;
	JDialog frm;
	...
	public PartyCreationViaScreenP(JDesktopPane desk)
	{
			...
			if(partyTf.getText().trim().length()>0)
			{
	        		    		...
	        		    		else			
    	        				{
			            			...
			            			if(drCrCb2.getSelectedItem().toString().equals("Dr"))
			            				wt="-"+wt;
			            			st.executeUpdate("insert into partyDetails values('"+partyTf.getText().trim().toUpperCase()+"','"+addressTf.getText().trim()+"','"+contactTf.getText().trim()+"','"+amt+"','"+wt+"','"+amt+"','p')");
			            			
    	        					JOptionPane.showMessageDialog(null,"New party has been added successfully.");
			            			//reset();
			            			Purchase.partyCb.removeAllItems();
			            			rs=st.executeQuery("select * from partyDetails where p_type='p' order by partyName ");
        		    				while(rs.next())
        		    				{
				        		    	Purchase.partyCb.addItem(rs.getString("partyName"));        		    	
        						    }
        						    Purchase.partyCb.setSelectedItem(partyTf.getText().trim().toUpperCase());
		           					frm.dispose();
        		    			}
            					//cn.close();            			
