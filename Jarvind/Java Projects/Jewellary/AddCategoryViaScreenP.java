...
public class AddCategoryViaScreenP extends JFrame implements ActionListener,MouseListener,KeyListener
{
	Container con;
	JDialog frm;
	...
	public AddCategoryViaScreenP(JDesktopPane desk)
	{
		frm=new JDialog(new Frame(),"Add New Category",true);
			...
			try
			{
				if(categoryTf.getText().trim().length()>0)
				{
        		    ...
        		    else			
            		{
    	        		...
    	        		Purchase.categoryCb.removeAllItems();
    	        		Purchase.categoryCb1.removeAllItems();
    	        		rs=st.executeQuery("select * from categoryList order by categoryName");
        		    	while(rs.next())
        		    	{
        		    		String str=rs.getString("categoryName");
        		    		Purchase.categoryCb.addItem(str);
        		    		Purchase.categoryCb1.addItem(str);        		    	
        		    	}
		           		Purchase.categoryCb.setSelectedItem(categoryTf.getText().trim().toUpperCase());
		           		Purchase.categoryCb1.setSelectedItem(categoryTf.getText().trim().toUpperCase());
		           		frm.dispose();
	            		//categoryTf.setText("");
            			//categoryTf.requestFocus();
            			//getCategoryName();            			
