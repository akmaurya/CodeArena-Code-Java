...
public class ShowRokarInDateRange extends JFrame implements ActionListener,KeyListener//,MouseListener,FocusListener
{
	...
	public ShowRokarInDateRange(JDesktopPane desk)
	{
		...
		dateTf1.setBounds(205,10,130,20);
	...
	private void showData()
	{
		try
		{
			...
			System.out.println("select p_name,s_s_no,s_date from sale where s_date>=#"+dt+"# and s_date<=#"+dt1+"# group by s_s_no,p_name,s_date order by s_date");
            ResultSet rs1=st1.executeQuery("select p_name,s_s_no,s_date from sale where s_date>=#"+dt+"# and s_date<=#"+dt1+"# group by s_s_no,p_name,s_date order by s_date");				
			while(rs1.next())			
			{
            ...
            rs1=st1.executeQuery("select p_name,p_s_no,p_date from purchase where p_date>=#"+dt+"# and p_date<=#"+dt1+"# group by p_s_no,p_name,p_date order by p_date");				
			while(rs1.next())			
			{
				String arr[]=new String[6];
				arr[0]=rs1.getString(1);	//party name
			...
			while(rs1.next())			
			{
				...
				//IN
				ResultSet rs2=st2.executeQuery("select * from ac_transaction where db_party='"+dbParty+"' and trans_date>=#"+dt+"# and trans_date<=#"+dt1+"# order by trans_date");	
				while(rs2.next())
				{
				...
				//OUT
				rs2=st2.executeQuery("select * from ac_transaction where cr_party='"+dbParty+"'and trans_date>=#"+dt+"# and trans_date<=#"+dt1+"# order by trans_date");	
				while(rs2.next())
				{
					String voucherno=rs2.getString("voucher_no");
					double amt=rs2.getDouble("trans_amt");
