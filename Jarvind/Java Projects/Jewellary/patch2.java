			...
			for(i=0;i<psno.length;i++)
			{
				...
				if(rs.next())
				{
					...
					{					
						...
						w3=w3.setScale(3,5);
						w5=w5.setScale(3,5);
						System.out.println("update sale_x set field3="+w3+",field5="+w5+",field4="+f4+",field6="+f6+" where s_s_no="+psno[i]);
						st1.executeUpdate("update sale_x set field3="+w3+",field5="+w5+",field4="+f4+",field6="+f6+" where s_s_no="+psno[i]);
						f3=f5;
						f4=f6;
						
					}					
					System.out.println("i="+i+"psno="+psno[i]+";f3="+f3+";f4="+f4);
				}
			}
			if(psno.length>1)
			{
				st1.executeUpdate("update partyDetails set currentWt='"+w5+"',currentAmt='"+f6+"'  where 	partyName='"+pnm+"' and p_type='s'");
			}
