...
public class runRailwayCashClient extends Thread implements KeyListener,MouseListener
{
        JLabel imageViewer;
        Window window;
        Frame frame;
        static runRailwayCashClient gvw; 
        public runRailwayCashClient()
        {
                this.gvw=gvw;
        }
        //public static railway showGifFile(String gifFile,int imageHeight, int imageWidth)
        public runRailwayCashClient showGifFile(String gifFile,int imageHeight, int imageWidth)
        {
                gvw = new runRailwayCashClient(gifFile);
                gvw.centerAndSetVisible(imageHeight, imageWidth);
                return gvw;
        }

        public static runRailwayCashClient showGifFile(Image gifImage, int imageHeight, int imageWidth)
        {
                runRailwayCashClient gvw = new runRailwayCashClient(gifImage);
                gvw.centerAndSetVisible(imageHeight, imageWidth);
                return gvw;
        }
        public static void hideGifFile(runRailwayCashClient gvw)
        {
                gvw.removeNotify();
        }

        public runRailwayCashClient(Image  gifImage)
        {
                initialize(gifImage);
        }

        public runRailwayCashClient(Applet applet, String gifFileName)
        {
                Image gifImage = applet.getImage(applet.getCodeBase(),gifFileName);
                initialize(gifImage);
        }

        public runRailwayCashClient(String gifFileName)
        {
        ...
        {
                runRailwayCashClient object=new runRailwayCashClient();
                ...
                public void run()
                {
                        try
                        {
        						...
        						atm.Desktop_atm LF=new atm.Desktop_atm();
