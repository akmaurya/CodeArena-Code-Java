...
public class serverATM extends UnicastRemoteObject implements interfaceATM
{
		...
		try
		{
                        ...
                        for(int r=0;r<arr.length;r++)
			{
				...
				System.out.println("date....... ="+trandate);
                       ...
                       // System.out.println("totaldr="+totaldr+"   totalcr="+totalcr+"   balanceamt="+balanceamt);
		...
		try
		{
                        ...
                        for(int r=0;r<arr.length;r++)
			{
				...
				//System.out.println("part="+particulars);
                                ...
                                {
                                    indexString=indexString+r+",";
                                    debitamt=arr[r].substring(60,81).trim();
                                			//get debit amt
	...
	public String getSingleValue(String qry)
	{
            ...
            try
		{
                    ...
                    String qry="select particular,tran_debit_amt,tran_credit_amt,entry_no from ubi_bill_of_kgsg_cards where ubi_payment_no="+Paymentno+" and recon_status<>"+10+" and recon_status="+1+" order by entry_no desc";
                    System.out.println(qry);
                    rsD=stD.executeQuery(qry);
                    while(rsD.next())
                    {
                        String upart[]=rsD.getString("particular").split("/");
                      String uparticular=upart[0]+"/"+upart[1].substring(3, upart[1].length())+"/";
                      double udr=rsD.getDouble("tran_debit_amt");
                      double ucr=rsD.getDouble("tran_credit_amt");
                      int entry=rsD.getInt("entry_no");
                      qry="select entry_no from kgsg_n_ubi where tran_credit_amt="+udr+" and tran_debit_amt="+ucr+" and (particular like '"+uparticular+"%' ) ";
                    System.out.println(qry);
                    ...
                    if(item!=1)
                    {
                        ...
                        while(rs1.next())
                        {
                          String upart[]=rs1.getString("particular").split("/");
                          String uparticular=upart[0]+"/"+upart[1].substring(3, upart[1].length())+"/";
                          double udr=rs1.getDouble("tran_debit_amt");
                          double ucr=rs1.getDouble("tran_credit_amt");
                          int entry=rs1.getInt("entry_no");
                        ...
                        if(rs.next())
                        {
                            {
                               int en=rs.getInt(1);
                               qry="update kgsg_n_ubi set recon_status=2,ubi_payment_no="+Paymentno+" where recon_status="+0+" and entry_no="+en+"";
                               System.out.println(qry);
                               st2.executeUpdate(qry);
                               qry="update ubi_bill_of_kgsg_cards set recon_status=2 where recon_status="+1+" and entry_no="+entry+"";
                               System.out.println(qry);
                               st2.executeUpdate(qry);
                               flag=1;
                            }
                            ...
                            {
                                javax.swing.JOptionPane.showMessageDialog(null, "particuler="+uparticular,"Information",javax.swing.JOptionPane.INFORMATION_MESSAGE);
                                //javax.swing.JOptionPane.showMessageDialog(null, "particuler="+uparticular);
            ...
            {
                System.out.println("error in breakAmtKGSG method..."+ex);
            }
            System.out.println(flag);
            return flag;
        }
