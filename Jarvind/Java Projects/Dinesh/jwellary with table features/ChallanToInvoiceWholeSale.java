...
public class ChallanToInvoiceWholeSale extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener,InternalFrameListener,TableModelListener
{
	Container con;
	JInternalFrame frm;
	...
	public ChallanToInvoiceWholeSale(JDesktopPane desk,int billno)
	{
		this.desk=desk;
		frm=new JInternalFrame("Convert Challan To Invoice # Wholesale",false,true,false,true);
...
class ColorColumnRenderer extends DefaultTableCellRenderer 
{
				...
				//if(grid.getRowCount()>0 || grid1.getRowCount()>0)
				{
		    	   		    ...
		    	   		    for(int i=0;i<grid.getRowCount();i++)
		    	    	    {
								...
								if(v1.equals("null")==false && v1.equals("")==false)
								{
									...
									qry="insert into saleInvoice values("+psno+",'"+partynm+"','"+dt+"',"+itemcode;
									...
									if((""+grid.getValueAt(i,6)).equals("null")==true)
										st.executeUpdate("insert into LastTounch values("+psno+",'"+partynm+"',0,"+itemcode+",'s')");
									else	
									{
										Double tnch=Double.valueOf(""+grid.getValueAt(i,6));
										st.executeUpdate("insert into LastTounch values("+psno+",'"+partynm+"',"+tnch+","+itemcode+",'s')");
									}
							...
							String qry1="insert into saleInvoiceAmtDetails values("+psno+","+Double.valueOf(totalPurWtL.getText())+","+Double.valueOf(totalAmountL.getText())+","+Double.valueOf(totalPurFineWtL.getText())+","+Double.valueOf(totalSilverFineWtL.getText())+","+Double.valueOf(totalPurWtL1.getText())+","+Double.valueOf(totalAmountL1.getText())+","+Double.valueOf(totalPurFineWtL1.getText())+","+Double.valueOf(totalSilverFineWtL1.getText())+","+Double.valueOf(totalPaybleAmtL.getText())+","+Double.valueOf(vatAmtL.getText())+","+Double.valueOf(disamtvar)+","+Double.valueOf(netPaybleAmtL.getText())+","+Double.valueOf(depositeTf.getText())+")";
	...
	private boolean rollBackInvoiceData()
	{
		try
		{
			//Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
	   		...
	   		ResultSet rs,rsTemp;
	   		System.out.println("ok1 ...");
	   		rs=st.executeQuery("select * from saleinvoice where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='s'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("itemcode");
	   			String v6=rs.getString("grosswt");
	   		...
	   		rs=st.executeQuery("select * from saleinvoice where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='a'");
	   		while(rs.next())
	   		{
	   			int v2=rs.getInt("iTEMcode");
	   			String v6=rs.getString("grosswt");
	...
	*/
	...
	private boolean showBill()
	{
		try
		{
					String tableNameSaleXX="salechallan",tableNameSaleDetailsXX="saleChallanAmtDetails";
					...
					String partynm=""+partyCb.getSelectedItem();
        		    	...
        		    	reset();
					int btnflag=0;
								rs=st.executeQuery("select * from saleinvoice where s_s_no="+challanno+"");
								if(rs.next())
								{
									...
									tableNameSaleXX="saleinvoice";
									tableNameSaleDetailsXX="saleInvoiceAmtDetails";
									btnflag=1;
								}
