...
public class AddCategoryViaScreen extends JFrame implements ActionListener,MouseListener,KeyListener
{
	Container con;
	JDialog frm;
	...
	public AddCategoryViaScreen(JDesktopPane desk)
	{
			...
			try
			{
				if(categoryTf.getText().trim().length()>0)
				{
        		    ...
        		    else			
            		{
    	        		...
    	        		Sale.categoryCb.removeAllItems();
    	        		Sale.categoryCb1.removeAllItems();
    	        		rs=st.executeQuery("select * from categoryList order by categoryName");
        		    	while(rs.next())
        		    	{
        		    		String str=rs.getString("categoryName");
        		    		Sale.categoryCb.addItem(str);
        		    		Sale.categoryCb1.addItem(str);        		    	
        		    	}
		           		Sale.categoryCb.setSelectedItem(categoryTf.getText().trim().toUpperCase());
		           		Sale.categoryCb1.setSelectedItem(categoryTf.getText().trim().toUpperCase());
		           		frm.dispose();
	            		//categoryTf.setText("");
            			//categoryTf.requestFocus();
            			//getCategoryName();
