...
public class Sale extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener,InternalFrameListener
{
	Container con;
	JInternalFrame frm;
	...
	public Sale(JDesktopPane desk,int billno)
	{
		this.desk=desk;
		...
		totalPurWtL.setBounds(650,230,110,20);
		totalPurFineWtL.setBounds(760+230,230,110,20);
		...
		totalPurWtL1.setBounds(650,470,110,20);
		totalPurFineWtL1.setBounds(760+230,470,110,20);
		...
		billNoLb.setBounds(980,10,100,20);//1000,10,120,20);
		...
		jsp.setBounds(10,100,1110,120);
		jsp1.setBounds(10,325,1110,135);
					...
					Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
	   				Connection cn=DriverManager.getConnection("jdbc:odbc:jweldsn","zed","zed"); 
		    	    		    ...
		    	    		    for(int i=0;i<grid.getRowCount();i++)
		    	    		    {
									...
									String v7="s";
									int serialno=0;
									///ResultSet rs=st.executeQuery("select tnh from LastTounch where i_name='"+iname+"' and p_name='"+pname+"'  and t_type='s' order by srno desc");//and s_s_no<"+bno+"
									/*rs1=st1.executeQuery("select max(srno) from LastTounch where p_name='"+partyCb.getSelectedItem()+"' and i_name='"+itemCb.getSelectedItem()+"' and t_type='s' ");
									if(rs1.next())
									{
										serialno=rs1.getInt(1);
									}*/
									//"+(++serialno)+",
	...
	private void getLastTounch()
	{
			try
			{
        		    ...
        		    ResultSet rs=st.executeQuery("select tnh from LastTounch where i_name='"+iname+"' and p_name='"+pname+"'  and t_type='s' order by s_s_no desc");//and s_s_no<"+bno+"
        		    if(rs.next())
        		    {
	...
	private void showPreBill()
	{
		...
		try
		{
        		    ...
        		    if(rs.next())
        		    {
						...
						/*//Check each Updation
   	        		    rs=st.executeQuery("select s_s_no from sale where p_Name='"+partynm+"' and s_s_no<"+Integer.parseInt(billNoLb.getText())+" order by s_s_no desc");
   	        		    if(rs.next())
   	        		    {
   	        		    	int justLastBillNo=rs.getInt(1);
	   	        		    rs=st.executeQuery("select field5 from sale_x where s_s_no="+justLastBillNo+"");
	   	        		    if(rs.next())
	   	        		    {
	...
	private void showBill()
	{
		try
		{
        		    ...
        		    ResultSet rs;
					String partynm=""+partyCb.getSelectedItem();        		    
        		    rs=st.executeQuery("select * from sale where s_s_no="+Integer.parseInt(billNoLb.getText())+" order by s_s_no");
        		    if(rs.next())
        		    {
        		    	reset();
