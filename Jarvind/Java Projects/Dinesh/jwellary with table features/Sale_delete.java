...
public class Sale extends JFrame implements ActionListener,MouseListener,KeyListener,FocusListener
{
	Container con;
	JInternalFrame frm;
	...
	JComboBox drCrCb;	
	public Sale(JDesktopPane desk)
	{
		this.desk=desk;
		frm=new JInternalFrame("Sale",false,true,false,true);
	...
	private boolean isBillExist()
	{
		try
		{
			Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
	   		...
	   		ResultSet rs;
	   		rs=st.executeQuery("select * from sale where s_s_no="+Integer.parseInt(billNoLb.getText()));
	   		if(rs.next())
	   		{
	   			return true;
	   		}
	   		...
	   		{
	   			rs=st.executeQuery("select * from sale_x where s_s_no="+Integer.parseInt(billNoLb.getText()));
	   			{
	   				if(rs.next())
	   				{
	   					return true;
	...
	private boolean rollBackData()
	{
		try
		{
			Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
	   		...
	   		rs=st.executeQuery("select * from sale where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='s'");
	   		while(rs.next())
	   		{
			...
			rs=st.executeQuery("select * from sale where s_s_no="+Integer.parseInt(billNoLb.getText())+" and t_type='a'");
	   		while(rs.next())
	   		{
	   			String v2=rs.getString("i_name");
	   			String v6=rs.getString("f_wt");
        		...
        		if(rs.next())
        		{
        			...
        			if(rs.next())
        			{
        				...
        				System.out.println("rollback->"+"update partyDetails set currentAmt='"+(""+rs1)+"',currentWt='"+(""+gm1)+"' where partyName='"+partyCb.getSelectedItem().toString()+"'");
        				st.executeUpdate("update partyDetails set currentAmt='"+(""+rs1)+"',currentWt='"+(""+gm1)+"' where partyName='"+partyCb.getSelectedItem().toString()+"'");
        				rs=st.executeQuery("select * from partyDetails where partyName='"+partyCb.getSelectedItem().toString()+"'");
        				if(rs.next())
        				{
        					System.out.println(""+rs.getDouble("currentWt"));
        				}
        			}
        			...
        			if(rs.next())
        			{
        				...
        				System.out.println("rs="+s1rs+"      gm="+s2gm);
        				rs=st.executeQuery("select slx.field3,slx.field4,slx.field5,slx.field6,slx.s_s_no from sale_x slx,sale sl where sl.p_Name='"+partyCb.getSelectedItem().toString()+"' and sl.s_s_no=slx.s_s_no and slx.s_s_no>"+Integer.parseInt(billNoLb.getText()));
        				while(rs.next())
        				{        				
        					...
        					int id=rs.getInt(5);
        					System.out.println("update sale_x set field3='"+(""+gm1)+"',field4='"+(""+rs1)+"',field5='"+(""+gm2)+"',field6='"+(""+rs2)+"' where s_s_no='"+id);
        					stTemp.executeUpdate("update sale_x set field3='"+(""+gm1)+"',field4='"+(""+rs1)+"',field5='"+(""+gm2)+"',field6='"+(""+rs2)+"' where s_s_no="+id);
        				}	
        			}
        		}
			//}
			System.out.println("ok4 ...");
			st.executeUpdate("delete * from sale where s_s_no="+Integer.parseInt(billNoLb.getText()));
			System.out.println("ok5 ...");
			st.executeUpdate("delete * from sale_x where s_s_no="+Integer.parseInt(billNoLb.getText()));
			...
			if(dateTf.getText().trim().length()>0)
			{
				//if(grid.getRowCount()>0 || grid1.getRowCount()>0)
				{
        			    		...
        			    		if(updateFlag==false)
		    	    		    {
		    	    		    	...
		    	    		    	rs=st.executeQuery("select max(s_s_no) from Sale_x");
			    	    		    if(rs.next())
			    	    		    {
			    	    		    	int n=rs.getInt(1)+1;
		    		    		    	if(n>psno)
		    	    		    ...
		    	    		    for(int i=0;i<grid.getRowCount();i++)
		    	    		    {
									...
									String v7="s";
									st.executeUpdate("insert into Sale values("+psno+",'"+partynm+"','"+dt+"','"+v1+"','"+v2+"','"+v3+"','"+v4+"','"+v5+"','"+v6+"','"+v7+"')");
		    	    		    ...
		    	    		    if(contFlag==0)
		    	    		    {
		    	    		    	st.executeUpdate("insert into Sale values("+psno+",'"+partynm+"','"+dt+"','','',0,0,0,0,'')");
		    	    		    }
		    	    		    ...
		    	    		    if(pendingTb.isSelected()==true)
		    	    		    	...
		    	    		    	st.executeUpdate("insert into Sale_x values("+psno+","+d1+","+d2+","+d3+","+d5+","+d7+","+d9+","+d11+","+d12+","+d13+","+d14+","+d15+","+d16+","+d17+","+d18+",'"+rateType+"',"+d19+")");
		    	    		    	
									//update stock-->'partyDetails' table
		    	    		    	rs=st.executeQuery("select max(s_s_no) from Sale where p_name='"+partyCb.getSelectedItem()+"'");
		    	    		    	if(rs.next())
		    	    		    	{
		    	    		    		psno=rs.getInt(1);
		    	    		    	}
		    	    		    	/*rs=st.executeQuery("select max(slx.s_s_no) from Sale sl,Sale_x slx where slx.s_s_no=sl.s_s_no and sl.p_name='"+partyCb.getSelectedItem()+"'");
			    	    		    if(rs.next())
			    	    		    {
			    	    		    	int n=rs.getInt(1);
		    		    		    	if(n>psno)
									...
									rs=st.executeQuery("select * from Sale_x where s_s_no="+psno);
									if(rs.next())										
									{
										//System.out.println("Save->"+"update partyDetails set currentAmt='"+rs.getString("field6")+"',currentWt='"+rs.getString("field5")+"' where 	partyName='"+partyCb.getSelectedItem()+"' and p_type='s'");
										...
										System.out.println("Updation completion.");
									}
	...
	private void showPreBill()
	{
		try
		{
        		    ...
        		    ResultSet rs;
					String partynm=""+partyCb.getSelectedItem();        		    
        		    rs=st.executeQuery("select * from sale where p_Name='"+partynm+"' and s_s_no<"+Integer.parseInt(billNoLb.getText())+" order by s_s_no desc");
        		    if(rs.next())
        		    {
        		    	reset();
	...
	private void showNextBill()
	{
		try
		{
        		    ...
        		    ResultSet rs;
					String partynm=""+partyCb.getSelectedItem();        		    
        		    rs=st.executeQuery("select * from sale where p_Name='"+partynm+"' and s_s_no>"+Integer.parseInt(billNoLb.getText())+" order by s_s_no");
        		    if(rs.next())
        		    {
        		    	reset();
        		    	partyCb.setSelectedItem(partynm);        		    	
        		    	...
        		    	System.out.println("select * from sale_x where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
        		    	rs=st.executeQuery("select * from sale_x where s_s_no="+Integer.parseInt(billNoLb.getText())+"");
        		    	if(rs.next())
        		    	{        		    		
	...
	class printbill extends JFrame 
    {
                public printbill()
                {
                        try
                        {
								...
								rs=st.executeQuery("select * from Sale where s_s_no="+Integer.parseInt(billno)+" and t_type='s'");
		    	    		    while(rs.next())
		    	    		    {
		    	    		    ...
		    	    		    rs=st.executeQuery("select slx.field3,sl.s_date from Sale_x slx,Sale sl where slx.s_s_no="+Integer.parseInt(billno)+" and sl.s_s_no=slx.s_s_no");
		    	    		    if(rs.next())
		    	    		    {
		    	    		    	temp=""+rs.getDate(2);
		    	    		    	l=temp.length();
								...
								rs=st.executeQuery("select * from Sale where s_s_no="+Integer.parseInt(billno)+" and t_type='a'");
		    	    		    while(rs.next())
		    	    		    {
		    	    		    	temp=rs.getString("i_name").trim()	;
		    	    		    	pw.print(temp);
		    	    		    ...
		    	    		    rs=st.executeQuery("select slx.field9,slx.field10,slx.field11,slx.rate_type from Sale_x slx where slx.s_s_no="+Integer.parseInt(billno)+"");
		    	    		    if(rs.next())
		    	    		    {
								...
								//space="    0.00       0.00";
        					    //pw.print(space);
		    	    		    rs=st.executeQuery("select field4,field6,jama,field11 from Sale_x where s_s_no="+Integer.parseInt(billno)+"");
		    	    		    if(rs.next())
								{
